# 开发执行控制计划（历史归档，含 cef712b 后未提交的审计草稿）
...
- `current_next`: **暂无 `NEXT`**（`HOST-ADMISSION-DESIGN-001` 已由 `03eec4a` 收卡为 `DONE`，**且只覆盖设计交付**：
  ...
  **registry 一字未动**：1.20.1 的 `STOP_PHASE_EXPLICIT_KEY_RELEASE` 那一格现在有 Bridge 自己写下的工件，
  ...
  `terminate()` 客户端进程（`adapters/launcher/orphans.py:314-353`），而 Core 的显式松键要到
  `cli/session_runtime.py:423-438` 的 wind-down 才发出——那时 Bridge 已被这次停止终止，两跑皆
  `input_release_failed: true`。同一 run 里 `move_input_was_leased` 与 `the_server_saw_the_kin_stop_after_the_move`
...
- `last_checkpoint`: **HOST 支线的边界从"没读到底"变成"读到底且剩下三格所有权"**——
  ...
  **是否据此划出缺口、是否提升 V08 仍归主控**；计划自本提交起**暂无 `NEXT`**。
  ...
  Core 的 `ReleaseAllInputs(EXPLICIT)` 在 wind-down 里才发（`session_runtime.py:423-438`，两跑 `input_release_failed: true`）。
  ...
  **2026-09-25 用户已答此题**（修正停止顺序：先让仍存活的 Bridge 确认松键，再终止客户端），该决策由本提交以
  ...
  `faeec4a9…`、recipe `bb456060…`、plan `9e0e0ccc…` **一字未动**，其黄金证明 `dd1e49ce…` 仍是正例、任一版本字段动一格
  即被同一 seam 拒。1.20.1 candidate 重建（jar `e50d61c2…` / 1,310,604，Windows 两次与 Linux 容器冷构建三次同一摘要）
  并把 `V1201-010/020/040/070` + 1.21.4 的 `CORE-010` 在受控 runner 上各重跑一次、逐个四读一致，再按实测把
  registry 引用换成新 build 真件（`status`/`capabilities`/`gaps` 未动）。随之移动的摘要逐条列在 `moved_digests_hello`；
  1.21.4 root 仍写死常量这件事按停止条件另登 `BRIDGE-1214-RUNTIME-IDENTITY-001`，**其 `status` 与晋升不属本卡**。
  **再此前**，跨版本路线走到 tested 门复判收口——审查卡 `VERSION-TESTED-GATE-AUDIT-001` 以六行结论
  ...
  **上一轮** V05 `VERSION-RESOLVER-001` 交付的是纯领域解析边界：
  ...
  `tests/fixtures/registry/reviewed-tested-bundles.json`（当时摘要 `d9e4823b80f5…`，现已随
  `VERSION-BRIDGE-IDENTITY-001` 换代为 `6aaa63420eee…`，见该卡 `moved_digests_hello` 6；以**新增行**进
...
- **同一规则下核的第六类现在时声明：文档写下的证据摘要（run id / bundle 摘要）配不配得上 registry**（2026-09-26，读数所在线
  ...
  **本轮只动文档与 `.tmp` 校验脚本**，产品代码、测试与夹具一字未改；九道门与 pytest 的实测读数记在
  `docs/development-todo.md` 同名一节，且都在最后一格 docs 改动之后跑。
- **同一规则下核的第七类现在时声明：计划元数据与交接文档里那些「仍 / 今天 / 暂无」的句子，还对不对得上现算的机器读数**
  （2026-09-26，读数所在线 `cef712b`）。前六格判的是工件与引用，这一格判**散文里的状态**。作用域是两段：本文
  `## 计划元数据`（第 8–375 行那一段）与交接文档全文。六条句式各对应一个可现算的不变量——① `planning_gaps` 仍只有
  `PERSIST` 一条 `UNFROZEN_CASE_IDS` ⇔ `tools/report_cases.py` 的 `requirements.planning_gaps` 恰为那一条；
  ② `HOST/W80+` 仍 `DEFERRED` ⇔ 卡片自己的 status 行（读**卡面**，不读这句散文）；③ `….json` 仍未创建 /
  仍不存在 ⇔ `git ls-files`（全路径按精确路径判，裸名按尾名判）；④「暂无 `NEXT`」⇔ `.tmp/count_plan_cards.py` 现数；
  ⑤「今天没有任何一条缺口能在不真实运行的情况下关掉」⇔ `local-only ∩ missing == ∅`；⑥「N 条 `local-only` required
  全为 `present: true`（名单）」⇔ 条数、逐条 `present`、名单集合三者。**判据实读**（`.tmp/check_plan_state.py`）：
  `sections: 2 / claims judged: 10 / data-root claims left unjudged: 11 / findings: 0 / exit 0`，
  逐族 `ABSENCE=2 CARD_STATUS=2 GAP=1 LOCAL_GAP=1 LOCAL_LIST=2 NEXT=2`，机器侧
  `planning_gaps=['PERSIST'] local-only=7 local-only∩missing=0 NEXT=0`；十条锚点落在本文 21/26/26/37/39/42 与交接文档
  10/24/24/37。**十一道变异与对照读数**（`.tmp/reverse_plan_state.py`，输出留在 `.tmp/plan-state-reversals.txt`，
  「bad cases: 0」）：R1 把唯一 gap 写成 `V08` → `STALE_GAP`；R2 卡片状态写 `DONE` → `STALE_CARD_STATUS` ×2；
  R3 把「仍未创建」挪到仓里真有的 `reviewed-tested-bundles.json` 上 → `STALE_ABSENCE`；R4 句子一字未改、只把卡片
  清点副本里一格 `status` 翻成 `NEXT` → `STALE_NEXT` ×2；R5 只改读数（`CORE-001` 进 `missing` 且 `present` 翻
  `false`）→ `STALE_LOCAL_GAP` + `STALE_LOCAL_LIST` ×2；R6 只把名单少写一个名字 → `STALE_LOCAL_LIST` ×1；
  R7 六类句式全部改写掉 → `CLAIM_NOT_FOUND` ×7（通用一条 + 每族一条）**加** `SCAN_TOO_NARROW`，即守卫不能因为
  「没找到可判的句子」而绿；R8 只拆一句 → 精确报缺那一个家族；R9 `FLOOR` 抬到命中数之上 → 只红 `SCAN_TOO_NARROW`；
  R10 是**不滥报的对照**：翻掉一个 `runtime-required` case 的 `present`，十条声明仍全绿（判据认得 `validation_class`）；
  R11 positive control = 真扫描、现算 `uv run --frozen python tools/report_cases.py`、不喂任何缓存，与缓存基线逐字相同。
  **这一轮又抓到自己的两处判据错**（都是变异红在别的理由上暴露出来的，不是我重读出来的）：第一处，① 式的面名先写成了
  `[A-Z]+`，于是 R1 期望的 `STALE_GAP` 变成 `CLAIM_NOT_FOUND family=GAP`——真值 `PERSIST` 恰好全字母，**换成带数字的
  面名才证明这条判据原本根本读不到**，已放宽为 `[A-Z0-9+/_-]+`；第二处，反证脚本复刻作用域时把两段并成一段，`暂无 NEXT`
  的「每段只判第一条」因此少判一条（`NEXT=1` 对基线 `2`），这条差异被 R0 那道**逐字对照**当场抓住，改由
  `<!-- SECTION -->` 分段复刻后才等价。**三条按设计留下的边界**：钉了 SHA 或日期的读数（「在 `55d0ecc` 上读出
  `required: 74`」）是历史快照，不追溯判；需要数据根 `/data` 的 11 条 `promotable:` 散文只计数、不判；「暂无 `NEXT`」
  在两段里共出现 14 次（本文 5 / 交接 9），只判每段第一条即当前 `current_next` 锚点，其余 12 次都在写了日期或提交号的
  历史句里。交接文档第 ⑥ 条没有名单，故那一格只判条数与逐条 `present`。**本轮同样只动文档与 `.tmp` 校验脚本**，
  产品代码、测试、夹具、卡片状态一字未改；五把文档守卫仍在未跟踪的 `.tmp/`，**是否提进被跟踪的 `tools/` 并接进 CI
  归主控**。九道门与 pytest 的实测读数记在 `docs/development-todo.md` 同名一节，且都在最后一格 docs 改动之后跑。
...
- `self_review`: 两轴自审在 `eeac5b0` 之后做，结论与修复都落在 `8498084`。
  ...
  - **仍未闭合**：冻结 schema 仍只描述 kill 一类（`ADMIT-070-RECORD-SCHEMA-001`）；本卡的两次
    ...
    （**2026-09-26 追记，只指路不改上面这句当时的读数**：第一项已由 `de8ed7d` 闭合，见该卡
    `completion_readings_2026-09-26`；第三项「新 jar 字节的 Linux 逐字节复现」仍未做。）
...
- `status`: `DONE`（实现 `de8ed7d`，卡片文本在 `cc9ced2` 收卡、两条 ref 与远端 SHA 已核，GitHub Actions
  ...
  `allOf/if/then` 写完，reader 一字未动。宁可各说各话的那条退路没有用上。）
- `baseline_sha`: `ff63746`（登记提交）；领取时实际 checkout = 本提升提交。
- `promotion_reason`: 三条——依赖皆 `DONE`（`order` 第 3 个场景的封证 `ADMIT-070-CASE-001` 与注册它的
  `ADMIT-070-REFUSAL-INJECTION-001` 都已收卡，登记时那句"不替换当前唯一 `NEXT`"已到期）、缺口被测试钉住
  且属证据一致性而非新的 1.21.4 功能专题（写下来的 schema 与执行它的 reader 各说各话）、1.20.1 主线的
  下一格（`STOP_PHASE_EXPLICIT_KEY_RELEASE` 是否据此划出、是否提升 V08）**归主控决策**而不是本执行者可
  代决的工作项，因此它是当时唯一可执行的 `QUEUED`。`79cf8ce` 单独提升、`de8ed7d` 交付，两个提交分开，
  满足交接第 4、6 项。
...
- `stop_conditions`: 若 `jsonschema`（测试期依赖，draft 2020-12）无法在不往 reader 里加规则的
  ...
  让写下来的契约与执行它的读取器各说各话。
- `completion_readings_2026-09-26`（实现 `de8ed7d`；`LOCAL_ONLY`，本卡无真实运行，全部读数在本地测量）：
  1. **写下来的形式现在分两类**：根 `oneOf` 指向 `$defs.process_sigkill_record` 与
     `$defs.client_report_request_record`。`category` 缺省仍是 kill 一类（卷内每一张旧封存照读不变），
     明写 `PROCESS_SIGKILL` 的 kill 也接受，`CLIENT_REPORT_REQUEST` 走第二支；`case`、`attribution`、
     `reasons` 与两个单调时钟字段收进 `$defs` 由两支共用——reader 里一份的规则，文件里也只有一份。
     `$id` 与 `title` 仍是 v1：reader 的 `schema_version` 仍是 `1`，两类共用同一版本号。
  2. **`acceptance` 第①格（两类全部结构反例一致）**：`structural_mutations()` 由
     `kill_structural_mutations()`（10 件）与新增 `request_structural_mutations()`（13 件）合成 23 件，
     `test_the_schema_refuses_everything_the_reader_refuses_structurally` 逐件双读——reader 必须报出代码、
     schema 必须有错。本卡 `scope` 点名的四条都在其中：`EFFECT_WITHOUT_REQUEST`
     （`value:"0"` + `asked:false` + `observed:true`）、`INVALID_CATEGORY`（`category:"MAYBE"`）、
     `INVALID_ATTRIBUTION`（`generation:0`）、`asked` 由 `value` 派生（`yes-please`、`"0"`+`asked:true`、
     `"1"`+`asked:false` 三种矛盾各一件）。
  3. **反方向也量了（写下的形式不能比 reader 更严）**：新增
     `test_the_schema_accepts_every_shape_the_reader_accepts` 过 7 种真实形状——注入成功的 kill、
     没落地的 kill、明写类别的 kill、argv 里带空 option 值的真实客户端身份、被观察到的请求、
     未被观察到的请求、以及没人要过的（`value:"0"`）请求。让 schema 变严正是「空 argv 元素那次把
     客户端 kill 和它的拒绝理由一起变得写不下来」的复现路径，这条测试把它钉住。
  4. **两类的边界是一条断言**：`test_the_schema_names_both_kinds_and_refuses_a_record_that_names_neither`
     拒掉借字段的两份文档（请求加 `outcome:INJECTED`、kill 加 `request`/`effect`），并确认缺 `signal`
     的 kill 不会被改读成请求——否则一次没落地的击杀会伪装成一次没发生的询问。
  5. **仍归 reader 的两处比较被说出来**：`test_the_two_comparisons_the_reader_still_keeps_for_itself`
     量到 `REASONS_MISSING` 与 `REASONS_NOT_FOR_THIS_OUTCOME` 由 reader 拒、schema 沉默——和被替换掉的
     钉缺口测试同职，但钉的是「哪里仍然分工」（与 kill 一类的 outcome-vs-confirmation 同一条线），
     不再是「哪里还缺」。
  6. **反证与 positive control**：`uv run --frozen python .tmp/schema_counterexamples.py` 把新写的 7 条
     规则逐一从盘上的 schema 里删掉再跑那 4 个具名测试——7/7 变红（`exit=1`），未删时 positive control
     `4 passed` `exit=0`，脚本按字节还原并复核 digest 前后一致（`34e6d26c…`）。整段读数在
     `.tmp/schema-counterexamples-final.log`。
  7. **`acceptance` 第②③格（fixture 侧，并更正登记卡的前提）**：动的只有
     `tests/fixtures/manifest.sha256:7` ——`cc983bc8fc4b12c9bf9725b50e9d31493941c83066b93cb4aab2ff293b3be0a0`
     → `34e6d26c7d102121cde10d9b43c6eb99951746a80993abce4e41d4a717802e19`（用门禁自己的测量函数
     `tools/verify_fixture_digests.py` 的 `_normalized_bytes` 量出，改前该工具报的正是这一行）。
     **`w00-contract-001.json` 一字未改**：`case_version` 是 case 文档自身的 sha256
     （`src/minekin_core/domain/cases.py:731`），文档没动则新旧同为
     `c59b9636646e3e3d3964d51421c3ead556f12a5936bbea74c46fbfc89f6b66b4`（改前改后各量一次，并与
     `git show HEAD:tests/fixtures/cases/w00-contract-001.json` 比字节相等）。所以登记时那句
     「改 schema 等于给一张无关的 case 重新定版」不准确：真正的耦合是 `fixture_digests_match_manifest`
     那条断言会读 manifest 第 7 行——不重签就是 W00 判红，而不是 case 悄悄换版。另核：旧 digest 在全跟踪树
     里已无出现（`cc983bc8fc4b12c9` grep 无命中），`tests/fixtures/registry/reviewed-tested-bundles.json`
     只钉 `bridge_digest`/`launch_plan_digest`/`recipe_digest`/`bundle_digest` 四类，**没有任何 `tested`
     声明被本卡移动**。
  8. **`acceptance` 第④格（本地全量门禁，`bash .tmp/run_local_gates.sh` 于 `de8ed7d` 干净工作树）**：
     `ruff check` 0、`ruff format --check` 0、Pyright `0 errors`、`check_case_assertions`
     `OK (140 registered)`、`verify_fixture_digests` `W00 schema and fixture digests: OK`、
     `check_boundaries` 0、`check_workflow_pins` 0、`git diff --check` 0；全量 pytest
     **`2500 passed / 2 skipped`**（两处跳过仍是既有的平台限制）。定向文件
...
- `depends_on`: `EXPLICIT-RELEASE-AT-STOP-001`（判官侧 token 与 case `V1201-080` 已在 `f38cf34` 就位；
  那张卡自身的 `forbidden_paths` 不因本决策放宽，工件由本卡取得）、`VERSION-SESSION-SWITCH-001`
  （被改的正是 `session stop` 与 wind-down 的次序）。
- `question`: 能否让 `session stop` 在终止客户端**之前**先向**仍活着**的 Bridge 发出显式
  `ReleaseAllInputs(reason_code=EXPLICIT)` 并**得到确认**，使 1.20.1 在**不改一条判据**的前提下把
  `V1201-080` 封成 PASS（客户端日志一行
  `bridge released N input(s) after CORE_REQUEST (EXPLICIT)`，N>0），同时保持"身份未证明的 PID 绝不被信号"、
  run document 的字段集、以及三种既有释放（`TIMEOUT` / `LEFT_PLAYABLE` / `IPC_LOST`）的语义不变？
- **领取前量到的现场**（2026-09-25，基点 `f38cf34`，全部是读代码或真跑读数，不是推断）：
  1. 今天的停止是一条**不带询问**的终止：`bootstrap.py:323 → cli/session.py:1560-1582 →
     adapters/launcher/orphans.py:314-353`，终步 `terminate(pid)` 打在**客户端进程**上；而
     `RELEASE_ALL_INPUTS` 全仓只有一个发送者（`cli/session.py:1370`，经 `:1433` 以 EXPLICIT 调用），
     它运行在 `cli/session_runtime.py:423-438` 的 `finally` 里、即 `connections.close()`（:432）**之后**。
     两次真跑（run `6d11ab7d…` / `ffdd54fc…`）都记 `input_release_failed: true` 且客户端日志里 `released` 计数为 0。
  2. 会话侧已有**可复用的同形状先例**：`until_input_release` / `on_input_release`（`session.py:1390-1419`）
     正是"时刻由调用方掌握、运行时照做且**不结束 run**"的分支，`session_runtime.py:368-402` 的 `branches`
     本就支持任意多条这类分支。"操作员要求停止"就是缺的那一条时刻。
  3. 客户端进程由会话自己持有的 supervisor 管理（`adapters/launcher/supervisor.py:87/153/163` 的
     `running()` 与 `stop(timeout_s)`），而今天 `cli/` 里**无人调用** `supervisor.stop()`——终止权在
     `session stop` 那条命令手里。本卡**保持**这一分工：谁终止不变，只改**次序**。
  4. `session stop` 与活会话是两个进程，今天之间**没有任何通道**。marker 在
     `<run_root>/session/<session_id>/generation-<n>/process.json`（`orphans.py:101-117`、`_markers` :119-124），
     它已含 `session_id` 与 `generation` ⇒ 请求该投递到的地址今天就查得到。
  5. 判据侧不需要改动：`V1201-080` 三条断言在 attempt 2 里已是"两条 observed + 只缺本卡那一条"
     （`move_input_was_leased`、`the_server_saw_the_kin_stop_after_the_move` 都成立）。
  6. 回归面实测：`tools/report_promotion.py:204-222` 的 `from_repository_build` 只把 bundle 记的 launch plan
     摘要与本检出按该版本算出的摘要相比（后者由 recipe / bridge jar / source tree 决定），Python 改动不移动它
     ⇒ 被 registry 引用的 run **不需要**重封；`the_lease_expired_and_was_released`
     （`tools/assert_case_evidence.py:1591-1602`）取 reason **集合**含 `TIMEOUT`，故停止阶段多出的一条
     `INPUT_RELEASED(EXPLICIT)` 不改变 V1201-040 的结论——但这一格仍要真跑复证（见 `acceptance` 第 4 格）。
- **要做成的形状**（设计决策写进卡，实现按此落地）：
  - 新增一对文件：**停止请求** `<run_root>/stop-requests/<session_id>-generation-<n>.json`
    （含 `request_id`、`requested_at`）与其**回执**同名 `.ack.json`（回同一 `request_id`）。放在 run root 下
    独立的 `stop-requests/` 目录，**不**写进 overlay（overlay 的既有内容属会话状态）。
  - `session stop`：对每个**已证明是本 run 记录的**客户端 PID（身份证明与三态归类规则一字不改），先按 marker
    给出的 `session_id`/`generation` 写请求，再在**有界等待**内等回执；等到 → 报告"松键在终止前已确认"；
    等不到（无活会话应答 / 超时）→ 照今天的路径终止并**如实报告未确认**。退出码仍只由 `outcome.complete` 决定。
  - 活会话：`session_runtime` 多接一对 `until_stop_request` / `on_stop_request`（与既有两条同形状，触发后
    **不结束 run**）；`on_stop_request` 走既有的 `release_inputs(arbiter, ReleaseReason.EXPLICIT)`
    （`session.py:1360-1388`：同一函数、同一字面 reason、同一 ledger 记账），发送成功后写回执。
  - `on_wind_down`：**当且仅当**本次 run 已按停止请求把 EXPLICIT 释放送达时跳过重发；其它任何结束原因下仍是
    无条件的安全网（`session.py:1421-1428` 注释所依据的理由不被削弱）。
  - **不新增 run document 字段、不动 `schema_version`**：确认与否由 `session stop` 自己的输出、ledger 的
    `INPUT_RELEASED` 行与 Bridge 的日志承担。
- `scope` 与 `allowed_paths`（显式清单）：
  - `src/minekin_core/adapters/launcher/stop_request.py`（新）：请求/回执两份文档的写入与容错读取
    （读不到 / 不可解析 / schema 不符 ⇒ 视为"没有请求"，不抛给操作员）；
  - `src/minekin_core/cli/session_runtime.py`：仅**新增**一对 caller-owned 分支参数及其取消/失败记账，
    不改既有分支语义；
  - `src/minekin_core/cli/session.py`：会话侧 `until_stop_request` / `on_stop_request`、`on_wind_down` 的
    条件跳过、`stop_session()` 的"先问后杀 + 超时回落 + 如实报告"；
  - `tests/unit/test_stop_request.py`（新）与既有会话/停止侧测试文件的**新增**用例；
  - 本计划、`development-todo.md`、`qoder-execution-handoff.md` 的状态与读数。
- `forbidden_paths`: 判据一字不改——`tools/assert_case_evidence.py`、`tools/check_case_assertions.py`、
  `tests/fixtures/cases/**`（含 `v1201-080.json`）、`tests/fixtures/manifest.sha256`；
  registry `tests/fixtures/registry/reviewed-tested-bundles.json` 的任一字（`STOP_PHASE_EXPLICIT_KEY_RELEASE`
  是否划出仍归主控）；`bridge/**`、`bridge-1201/**`、`proto/**`（两 root 的松键与日志形状本就对）；
  `adapters/launcher/orphans.py` 的**身份证明与终止判定规则**（`prove_process_identity`、
  `stop_recorded_clients` 的三态归类、`terminate_process`）——本卡只在其之前加一次询问；
  `cli/auto_session.py` 的版本切换停止路径（见 `non_goals`）；run document 的字段集与 `schema_version`；
  既有封存 bundle 与 attempt 序号（重试领新序号）；V1201-060 的 `IPC_LOST` 那一格；用户的真实远程服（属 V08）；
  `PROCESS-RECOVERY-001` / `OPERATIONS-RETENTION-001` 的残留进程与 marker 策略。
- `acceptance`:
  1. 单元证据覆盖四条行为且**非空转**：(i) 请求在场 → 会话在 Bridge 仍活时发出 EXPLICIT 释放并写回执；
     (ii) `session stop` 在 `terminate` **之前**看到回执（测试须能区分"先确认后终止"与"终止之后才确认"）；
     (iii) 无活会话 / 超时 → 回落到今天的终止并报告未确认；(iv) 已确认的停止之后 wind-down **不再重发**，
     而其它结束原因照旧发。把 (ii) 改回"直接终止"必须让测试红；
  2. 门禁全绿：`uv run --frozen pytest tests/unit`、`ruff check`、`ruff format --check`、`pyright`、
     `tools/check_case_assertions.py`、`tools/verify_fixture_digests.py`、`tools/check_boundaries.py`、
     `git diff --check`；
  3. 1.20.1 受控真跑一次（attempt 2 的同形状：`--hold-forward-seconds 180` + `MINEKIN_DOMAIN_SECONDS=60`），
     **判据不改**，封存 `client/latest.log` 出现 `bridge released N input(s) after CORE_REQUEST (EXPLICIT)`
     （N>0），以 `V1201-080` 封成 `result PASS`（新 attempt 序号），四读一致且 `from_repository_build: true`；
  4. 同形状再跑一次 **V1201-040**（lease 到期那一格）并 `result PASS`——修复会让停止阶段多一条
     `INPUT_RELEASED(EXPLICIT)`，这一跑是"旧结论没被改坏"的真跑证据，不是可选项；
  5. 至少三条反证各红在其**命名理由**上，且有一条 positive control `exit 0`；
  6. 独立 commit + push 到两条 ref，`git ls-remote` 核对。
- `counterexamples`（逐个必须打不中）: 把超时回落当确认；用 Core 的 `INPUT_RELEASED(EXPLICIT)` 记账顶替
  Bridge 那一行；用 `released 0` 充数；靠整体删掉 wind-down 的无条件安全网来"避免重发"；
  以"会话结束时 Bridge 恰好还活着"当成交；拿 V1201-060 的 `IPC_LOST` 工件顶替本卡的显式释放。
- `validation_class`: `PRODUCT_CHANGE_WITH_REAL_RUN`——改产品代码并有单元证据，结论仍只来自 1.20.1 上
  那一次真跑及其封存读数（外加 V1201-040 的回归真跑）。
- `stop_conditions`: 若活着的 Bridge 收到 `ReleaseAllInputs` 后**不写**那行（要改 Java 才写）→ 停在
  `BLOCKED_DECISION`、不改两 root；若"确认"必须靠改 run document schema 或 ledger 语义才做得到 → 停止并报告，
  不改判据也不扩字段；若 V1201-040 那一格因这次改动再也封不出 PASS → 停止，保留两条失败件，不削弱判据；
  真实运行连续三次重现同一外部阻断 → 停止并报告。
- `non_goals`: 不改 `cli/auto_session.py` 的版本切换停止（那里今天也是"直接终止"；切换阶段的松键若要有工件，
  另立一张，本卡不代为扩面）；不判定 V07 剩余缺口是否收口；不提升 V08、不连接用户远程服；
  不把 `STOP_PHASE_EXPLICIT_KEY_RELEASE` 从 registry 的 `gaps` 里划掉；不动 `USE_TARGET_BLOCK_CHANGE`；
...
- `status`: `BLOCKED_DECISION`（本提交按本卡自己的 `stop_conditions` **第①格**停下：判官侧已经交付并推到两条 ref
  （`f38cf34`，新 token + case `V1201-080` + 单元门禁全绿 + 非空转反证与 positive control），但两次受控 1.20.1
  真跑各封一个新 attempt 序号、都红在同一个命名理由 `RELEASE_NOT_LOGGED` 上；要拿到 Bridge 那一行**必须改产品代码
  里"停止"的次序**，而改哪一处、要不要改是用户/主控的产品决策，不在证据卡权限内。读数见
  `blocked_readings_2026-09-25`，失败材料一字未删。）
...
- `next_after_done`: 本卡收口后，1.20.1 的 `STOP_PHASE_EXPLICIT_KEY_RELEASE` 那一格有工件、但 registry 原样；
  **是否据此划出缺口、是否提升 V08 仍归主控决定**，本卡不代为登记下一张。
  （实际走向：本卡没有收到工件，停在 `BLOCKED_DECISION`；下面 `blocked_readings_2026-09-25` 第 7 格写明待决的是什么。）
- `blocked_readings_2026-09-25`（两次真跑的实测，全部读自卷内封存件与容器内复判，不是推断）：
  1. **触发的是第①格，不是第②格**。第②格问"harness 能否在 lease 到期前完成显式停止"——实测**能**：
     attempt 2 以 `--hold-forward-seconds 180` 配 `MINEKIN_DOMAIN_SECONDS=60`，停止落在 lease 未到期之前
     （`MINEKIN_SERVER_JAR=.tmp/mc-1.20.1-server.jar MINEKIN_DOMAIN_CASE=V1201-080 MINEKIN_DOMAIN_PROBE=Kin
     MINEKIN_DOMAIN_SECONDS=60`，完整命令见第 7 格）：封存的 `client/latest.log` 里 `released` 计数为 **0**
     （连 V1201-040 那种 `CORE_REQUEST (TIMEOUT)` 都没有），末两行是
     `bridge pressed move.forward` / `bridge applied 2f2adcf0894541a18f5ff0151e5fc614: holding [move.forward]`
     ⇒ 那一刻键仍被按着、lease 仍活着。第①格问的是"Bridge 侧根本不打这行、需要改产品代码才拿得到"——两次真跑都是它。
  2. **attempt 1（死亡干扰未排除）**：run `6d11ab7d35b64f83a9352ff120bceaa2`、bundle
     `6afda194861acb0dc5f43d09fedcbc2694854a83019dbc65ff7cd4a49445efe1`、`attempt_sequence 1`、`result FAIL`、
     `failures = ["the_bridge_released_the_input_when_the_session_was_stopped:RELEASE_NOT_LOGGED"]`、13 件工件。
     该 run 的 Kin 在停止前被 Slime 杀死（`client/latest.log:204 [15:52:59] Kin was slain by Slime`），
     全日志唯一含 `released` 的一行是 `:205 bridge released move.forward`（死亡界面自动松手），
     它不是 `released N input(s) after <REASON> (<CODE>)` 的计数形状 ⇒ token 报 `RELEASE_NOT_LOGGED` 正是其命名理由。
     run document 同报 `input_release_failed: true`、`outcome: BRIDGE_LOST`。
  3. **attempt 2（去掉死亡干扰的复现）**：run `ffdd54fcdf454938a7ec63d045860ea6`、bundle
     `931157feaae6cbc94cd31777feb15fe5de4380e24c438540f2e0a8d8a155119c`、`attempt_sequence 2`、
     同一 `failures` 项、`result FAIL`；`grep -ic released client/latest.log` = 0、`deathscreen` = 0；
     run document `input_release_failed: true`、`outcome: BRIDGE_LOST`、`session_state: STOPPED`、
     `connection_state: PLAYABLE`，`session stop` 自身报 `{"status": "stopped", "terminated": [235], "unresolved": []}`
     （235 即 run document 的 `pid`）。容器内对**两件**封存件逐条复判（`.tmp/er-audit.sh`，未入库的临时读者，
     内容与 `tools/rejudge_evidence.py` 同源）：
     `move_input_was_leased` 与 `the_server_saw_the_kin_stop_after_the_move` 两格在两跑里**都 observed**，
     只有本卡的新 token 缺 ⇒ 阻断是这一件工件本身，不是场景没跑起来。
  4. **为什么拿不到（读代码得到的结构性事实）**：`session stop` 的实际路径是
     `bootstrap.py:323 → cli/session.py:1560-1582 → adapters/launcher/orphans.py:314-353`，最后一步对**客户端进程**
     直接 `terminate(pid)`；会话自己的运行时要等 `reader`/`client` 结束才进入 `finally`
     （`cli/session_runtime.py:423-438`），那里**先** `connections.close()`（:432）、**再** await `on_wind_down()`
     （:433-435），失败即置 `progress.release_failed = True`（:436-437）——正是两跑读到的 `input_release_failed: true`。
     而 `RELEASE_ALL_INPUTS` 全仓只有一个发送者（`cli/session.py:1370`，经 `:1433` 以 EXPLICIT 调用），Core 也不安装
     信号处理器。⇒ 现有次序下"显式松键"命令发出时，它的接收方已经被**这次停止亲手终止**，Bridge 无从写下那行。
  5. **不是判据太严，也不是别处能借**：lease 到期（V1201-040 的 `CORE_REQUEST (TIMEOUT)`）、离开可玩态
     （`LEFT_PLAYABLE (PLAY_ENDED)`）、Core 被杀（V1201-060 的 `IPC_LOST`）这三种释放都发生在 Bridge **还活着**
     的时候，所以它们有工件；只有"操作员主动停止"这一格的发送点在 wind-down 之后。用 Core 的
     `INPUT_RELEASED(EXPLICIT)` 记账或服务端"停下来了"反推，都在本卡 `counterexamples` 里明令不算。
  6. **本卡已交付与未交付**（按 `acceptance` 逐格）：1 ✅（新 token 只收日志原话 `after CORE_REQUEST (EXPLICIT)`
     且 N>0，把 `EXPLICIT` 换成 `TIMEOUT` 的同一行被拒）；2 ✅（`pytest tests/unit` 2184 passed / 2 skipped、
     `ruff check`、`ruff format --check`、`pyright`、`check_case_assertions.py` `OK (140 registered)`、
     `verify_fixture_digests.py`、`check_boundaries.py`、`git diff --check` 全绿，均在 `f38cf34`）；
     5 ✅（五条拒绝理由各有非空转反证 + positive control `exit 0`）；6 ✅（判官侧独立提交已推到两条 ref）；
     **3、4 ❌ 且按现有产品代码不可取得**——`V1201-080` 至今没有 `result PASS` 的封存件，registry 的
     `STOP_PHASE_EXPLICIT_KEY_RELEASE` 仍留在 `gaps`（本卡未动 registry 任一字）。
  7. **交给用户/主控的决策**：(a) 把"停止"改成优雅版——Core 先向**活着的** Bridge 发
     `ReleaseAllInputs(EXPLICIT)`、确认后再终止客户端（改产品代码的一张实现卡）；(b) 承认 1.20.1"停止时松键"
     只能由 Core 记账 + Bridge 随进程消失 + 服务端推断来覆盖，据此改写或划出 registry 的
     `STOP_PHASE_EXPLICIT_KEY_RELEASE`（属主控对 `tested` 声明的决定）。
     **2026-09-25 用户已作答，选 (a)**：「修正产品的停止顺序，先让仍存活的 Bridge 确认松键，再终止客户端。」
     该决策由紧随提交落成 `GRACEFUL-STOP-KEY-RELEASE-001`（先 `QUEUED` 登记、再以下一个提交提升为唯一 `NEXT`），
     工件由那张卡取得；**本卡自身的 `forbidden_paths` 不因该决策放宽**——本卡不改产品代码，
     `V1201-080` 也仍在没有 PASS 的状态。
     登记那张卡时另测得一条与本卡第 7 格旧说法相反的事实：Python 侧的停止次序改动**不会**移动任何已封存 run 的
     `from_repository_build`（`tools/report_promotion.py:204-222` 只把 bundle 记的 launch plan 摘要与
     本检出按该版本算出的 launch plan 摘要相比，而后者由 recipe / bridge jar / source tree 摘要决定），
     所以"改产品代码就要把被引用的 run 全部重封"不成立；本卡旧文中"会牵动 1.21.4 的同一族判据与重封"以此更正。
     **保留的失败材料与复跑命令**：卷内两个 evidence 目录
     `/data/kin/kin-01/run/evidence/{6d11ab7d35b64f83a9352ff120bceaa2, ffdd54fcdf454938a7ec63d045860ea6}`、
     转录 `.tmp/explicit-release-v1201-080-run.log` / `.tmp/explicit-release-v1201-080-run2.log`；复跑形状：
...
- `status`: `DONE`（实现 `801f9ba`；本收卡提交把卡片按 `question` 的三格如实记账。两条 `stop_conditions`
  **都没有触发**：客户端运行时读到了自己真实的 loader 版本（`runtime_hello_reading`），1.21.4 的 jar / recipe / plan
  摘要一字未改（`faeec4a9…` / `bb456060…` / `9e0e0ccc…`），也没有用"两侧写死另一个常量"冒充修复。）
...
- `moved_digests_hello`（卡片点名"逐条列全"，全部旧→新）：
  1. 1.20.1 bridge jar sha256 `9e162d8359a886394ddd80db87477d9196ef3d2972e7a4d942df54a2f1e349bc`
     → `e50d61c209be98136216b34aadbb6d5a12db8def8aa63a536f32cda8e287006f`，size `1,308,469` → `1,310,604`；
  2. `source_tree_sha256`（candidate fixture 的 `source_digest`）
     `b5a817dda359ed5d350b814f53db45aa570ab7fc601c2ca150c65d0961ae1d20`
     → `ab33714672dc48a8ebd3f288a1e9ffe96ec11b9315a41a85ed26742b11eed979`；
  3. recipe 文件摘要 `f552b92aa9322b3cd2ff23fc0e927ff4b430e7d24974808e41341f412542c3bb`
     → `8ce43e26b2e17b10e161a9367b990b62a5350132dd81662a97b4e734a1be0ea0`
     （`tests/fixtures/manifest.sha256` 第 22 行同值替换）；
  4. launch plan 摘要 `ac40316094dda001c2333bdaea00f886609a58982153b085f276d91d750db8bc`
     → `83299ad5e224959de8e30c72c5937a2434c4d7bba92c62f4d22cf8d89f5f6181`（由 1–3 重算而来）；
  5. `src/minekin_core/adapters/launcher/recipe.py:71-72` 的 `BRIDGE_1201_JAR_SHA256` / `_SIZE` 常量随之改为
     `e50d61c2…` / `1_310_604`（其上方注释重述"两次构建同一摘要"的可复现依据）；
  6. registry `tests/fixtures/registry/reviewed-tested-bundles.json` 自身摘要
     `d9e4823b80f5ab0ed41f21448209517b260732d9bd9b13a231ec20b93d6270c7`
     → `6aaa63420eeee3af1920fdc0f208ab4d3d1b262c0dde8ca83ee1f7d1420dcd52`（manifest 第 83 行）。1.20.1 条目的
     `bridge_digest` / `launch_plan_digest` / `recipe_digest` 换成 1/3/4 的新值，四条引用整体换代：
     `V1201-010 bc203c0e…/13471d2f…` → `8665b021…/dc30bdaa…`，`V1201-020 87229052…/6dd17bd6…` →
     `ece5d0cb…/bc987a32…`，`V1201-040 f4cc67ae…/3d0ebebe…` → `155dcb4a…/435ab159…`，
     `V1201-070 5b1cbef5…/46aa3f00…` → `ecebb080…/eb054c0a…`；1.21.4 条目只有握手案 `CORE-010` 换代
     `e22cacf0…/6e0fc4fa…` → `73fec7d3…/98e32412…`，其 `bridge_digest` / `launch_plan_digest` / `recipe_digest`
     与其余十一条引用**未动**；
  7. `tests/fixtures/cases/core-070.json` 因 `tools/check_case_assertions.py --record` 重钉实现摘要而变：断言
     `no_input_is_replayed_after_an_ambiguous_disconnect` `3a44204850b9b45639d8bd7410ab4269d4e1356e0d1fce7431ead644565ab9b3`
     → `1560b0854480db15405133334f4505df3c4a50fb0ce936660d76cdb4bfc803ad`，文件摘要 `f467e42f…` → `1d583470…`
     （manifest 第 38 行同步）。
- `delivery_hello`（实现提交 `801f9ba`，21 文件 +353/−55）：Core 侧 `adapters/bridge/ipc.py` 给 `BridgeSession` 增加
  必填 `minecraft_version` / `fabric_loader_version`，`__post_init__` 以新增的 `VERSION_TEXT`
  （`^[0-9][0-9A-Za-z.\-_]{0,31}$`）拒形状不正的值——**因为证明上下文是这些字段 NUL 拼接后的 HMAC，允许分隔符
  就等于允许跨字段伪造**；`_validate_bridge_hello:451-452` 与 `_bridge_proof:642-643` 的字面量改为读 `session.*`。
  `adapters/bridge/bootstrap.py` 新增 `_bundle_version(plan, key)`：从已审 plan 的 `bundle.minecraft` /
  `bundle.fabric_loader` 取值，缺 bundle 段或取不到可用值即以 `MinekinError` 拒（"the launch plan has no bundle
  section" / "the launch plan has no usable bundle {key}"），再把它们交给 `bridge_session_for`。
  1.20.1 root 侧新增 `runtime/ClientRuntimeIdentity.java`（51 行，从 `FabricLoader.getInstance().getModContainer(
  "minecraft"/"fabricloader")` 读版本，任缺一件即 `orElseThrow` 失败关闭），`MinekinBridgeClient` 启动时取一次，
  `BootstrapDescriptorAdapter.adapt(descriptor, identity)` 把它送进 `Expected`，`HandshakeGate.java:169` 只接受
  `1.20.1`/`0.19.5` 这一对。**`bridge/`（1.21.4 root）字节未改**——它的 hello 仍是字面量，见 `not_tested_hello`。
  未触碰：其它握手字段的判据、Player-Equivalent / lease / 准入、地址策略、已冻结的 1.21.4 产物。
- `shared_selftest_fix_hello`（实现后的第二笔提交 `391cc2b`，CI 实测逼出来的）：`801f9ba` 推上去后 CI 的
  `bridge-static` job 在 `tools/check_bridge_protocol.py` 这步**红了**（run **#526/#527**，本地全绿但那条门没跑）——
  两个 root 共用的自测 `tools/java/BridgeProtocolSelfTest.java:323` 把 `1.21.4`/`0.16.9` 写死进 `Expected`，
  而 1.20.1 的 `HandshakeGate` 现在钉的是 `1.20.1`/`0.19.5`，于是同一份文件在 `bridge-1201` 下必然抛
  `game and Loader versions must match the 1.20.1 candidate`。修法是让**被检的 root 自己报名**：`tools/check_bridge_protocol.py`
  读该 root 的 `gradle/libs.versions.toml` `[versions]` 表并以 `-Dminekin.selftest.minecraft=` /
  `-Dminekin.selftest.fabric.loader=` 传给自测（表里缺任一键即 `SystemExit`），自测读到空值就以
  `AssertionError` 失败——**不留默认值**，否则又变回一处写死。另加一格断言：拿任一其它版本对构造 `Expected`
  必须被拒，钉"门只认自己 root 的那一对"。这条门改的是 `tools/`，不进 jar，封存摘要不受影响。
  实测（本机 JDK 21.0.12.1，走真实 checker 只改 `root_versions` 的返回值）：两 root 各按自己表内值 →
  `OK`；给 `bridge` 传 `1.20.1`/`0.19.5` → 拒；给 `bridge-1201` 传 `1.21.4`/`0.16.9` → 拒；不给属性 →
  `AssertionError`。CI 那三个 step（scaffold / 两侧 host boundary / protocol + proto-java）本地全序列复跑亦 `OK`。
- `local_gates_hello`（在实现提交前的同一棵树实测）：`uv run --frozen pytest` = **2439 passed / 2 skipped**
...
- `reseal_runs_hello`（受控 runner `minekin-runner:local` + 卷 `minekin-runner-data`，真 JVM、真 1.20.1 服务端；
  ...
  `V1201-010` run `8665b021bb594f64baa9256ccbc70366` / bundle `dc30bdaa0ac100bf6e7f25c1399d45340d342ac733b41c5fea134eaf85448494`
  （attempt 3）；`V1201-020` `ece5d0cb282946f786088bced9fbf4b1` / `bc987a3208a22a099a0effd29d94fdcd1c447585ed49846eb8970fc25533ad1f`（2）；
  `V1201-040` `155dcb4a527643618a6c09d94b39dce0` / `435ab159045b18f222a011f9acea96c849c29e271ce926df382e4899afdd8bc3`（2）；
  `V1201-070` `ecebb08084be468ea6372ba1d43c27d5` / `eb054c0af2aa91d81ad105061f0bd0bd0abc1371746ba9ae10836a09f925a0c8`（2）；
  1.21.4 侧握手正例 `CORE-010` `73fec7d3103e4e33baf7f8db857020f2` / `98e32412c326c6954625df31fe14bfba960417f9e8ca9603a0fc25df752a8b84`
  （attempt 2，supersedes `e22cacf088284598ad1928d426cc3a71`）。
- `runtime_hello_reading`（两侧各一次真跑的封存 bundle 内读数，脚本 `.tmp/v09_hello_evidence.sh`）：1.20.1 bundle
  （`V1201-020`）的 `client/latest.log` 第 1 行 `Loading Minecraft 1.20.1 with Fabric Loader 0.19.5`，且**整包**
  `1.21.4` / `0.16.9` 出现次数为 0；1.21.4 bundle（`CORE-010`）的客户端日志 `Loading Minecraft 1.21.4 with Fabric
  Loader 0.16.9`，`1.20.1` / `0.19.5` 出现次数 0；两份 `manifest.json` 各带自己的 `bridge_digest`
  （`e50d61c2…` / `faeec4a9…`）与 `launch_plan_digest`（`83299ad5…` / `9e0e0ccc…`），case 结果均 `PASS`。
  这就是"运行时读得到真实 loader 版本"这一停止条件的正面观测。
- `reproducibility_hello`：新 1.20.1 jar 三次构建同一摘要 `e50d61c2…` / 1,310,604——Windows `build`、Windows
  依赖校验严格 `build`、以及 Linux x86_64 容器内 `clean build`（`.tmp/v09-linux-cold-build.log`：现下 Gradle
  8.12.1、18 tasks executed、`checkHostBoundaryArtifacts` OK、`gradle/verification-metadata.xml` 仍为
  `23e10efa…`）。该日志末端的 `ImportError: StrEnum` 是容器 `python3`（3.10）跑校验脚本的措辞问题，与产物无关。
- `counterexamples_hello`（每种一个稳定类别）：**声明与本次 plan 不符即拒**——`test_a_session_with_one_version_of_the_pair_moved_does_not_share_that_digest`
  把 1.21.4 的黄金证明 `dd1e49ce34d4ce9662b48a7f35ead3910727244f7aae907081ab06514bdc0c5e` 原样送给字段动了一格
  的会话，经真实回环 seam 由 `BridgeIpcHost.authenticate()` 判为 `IpcProtocolError … was rejected`（两个参数化方向：
  minecraft、fabric_loader）；同文件保留"1.21.4 会话仍被它一直以来的摘要证明"的正例。plan 说不出发哪个 loader /
  根本没有 bundle 段 → bootstrap 拒。版本形状不可钉（空串、尾空格、含 NUL、`v` 前缀、超长）× 两个键 → bootstrap
  拒；直接构造 `BridgeSession` 持不可钉形状 → `ValueError`。Java 侧：`BootstrapDescriptorAdapterTest.
  anIdentityThatIsNotThisRootsRecipeIsRefused`、`HandshakeGateTest.onlyTheVersionPairThisRootActuallyRunsIsAccepted`。
  真件层反例是**供给链**那条：把 registry 指向日志里另一种 loader 拼写的 recipe（`.tmp/v09-candidate-wrong-loader.json`）后，
  容器内 `bundle install` 在抓取前拒 `SUPPLY_CHAIN / launcher.provision` 退出码 **11**（"digests to 9b1aacc9…, not the
  reviewed 8ce43e26…"）。被拒之后注册表照旧、无写副作用。
- `registry_boundary_hello`：本卡**没有**改 registry 的 `status`、`capabilities`、`gaps` 或 1.21.4 的 bridge/plan/recipe
  三条摘要，只按主控 2026-09-25 选定的路线把**引用**换成新 build 真件（"status 不动，全部引用换成新 build 真件"），
  并把它写进 `notes`：1.21.4 其余十一条引用是**更早 build 的封存 + 当前代码的复判**，这一点显式声明而非留给摘要说话。
  计划元数据第 20 行原本要求"重封后另卡登记 registry 更新"——主控指令优先于该措辞，偏差如实记在此格；
  `tested` 是否成立仍由 `TESTED-PROVENANCE-VERIFY-001` 用真件重算后判定。
- `not_tested_hello`：① **1.21.4 root 仍写死 hello 常量**——今天声明得对，但不是从运行时读来的；把它改成
  `ClientRuntimeIdentity` 同源做法会动已冻结的 `bridge_source_sha256` 与 `launch_plan_digest`，本卡的停止条件禁止，
  故另登 `BRIDGE-1214-RUNTIME-IDENTITY-001`（`QUEUED`）。② **"客户端真件自己谎报版本"的活体反例拿不到**：不篡改
  已封存 bridge 就产不出这样的件，所以该反例只有契约测试 + Java 测试 + 上面那条供给链真跑支撑；活的错误声明
  仍属未测。③ registry 的 `tested` 未经真件重算核对（`TESTED-PROVENANCE-VERIFY-001`）、1.20.1 停止阶段显式松键无
  独立工件（`KEY-RELEASE-AT-STOP-001`）、V06 三类 store 故障注入缺证据（`STORE-FAILURE-EVIDENCE-001`）、
  V07 的 `not_tested_v07` 缺口原样仍在。④ 用户的真实远程服未连接；look/move/松键的 V09/V10 动作本卡一律没做。
  ⑤ recipe 声明 Java 17 而封存用 21 的 `RUNNER_JDK_17_UNSEALED` 缺口未变。
- `docs_sync_hello`（摘要换代后，仓库里以**现在时**描述这些摘要的地方）：逐处只加带日期的更正括注，不改写历史
  读数——本文件元数据 `last_checkpoint` 里 V05 段的 registry 摘要；[跨版本计划](version-auto-to-server-control-plan.md)
  V04 段"允许登记的精确组合"（bridge/plan 两项摘要）与 V05 段 registry 摘要、V07 段握手缺陷现场；
  [执行交接](qoder-execution-handoff.md) 事实 ③ 与"当时能过握手"那句；
  [许可矩阵](version-license-matrix.md) 表格行与产物小节里"当前 jar 摘要/字节数"（并注明新 jar 只做过三次跨平台
  复现，V03 的第四次 `--rerun-tasks` 未重做）。各卡内部 `delivery_record_v0x` / 真实读数按登记时点冻结，保持原样，
  对账统一走 `moved_digests_hello` 的旧→新表。
- `next_after_done`: `TESTED-PROVENANCE-VERIFY-001`（已 `QUEUED`，其登记文字本就要求排在"本卡与其重封之后"；
  提升须下一次独立提交）。`BRIDGE-1214-RUNTIME-IDENTITY-001` 与 `KEY-RELEASE-AT-STOP-001`、
  `STORE-FAILURE-EVIDENCE-001` 同样排在其后；三卡 + V07 缺口全部收口前**不得提升 V08**，也不得把 reviewed
  registry 的 `tested` 当作远程入服的充分证据。
...
- `question`: 能否让 `bridge/`（1.21.4 root）像 1.20.1 root 一样从 Fabric 自己的 mod container 读取
  `minecraft` / `fabricloader` 并送进 `Expected`，同时把 1.21.4 candidate 重建、重封、按实测换掉 registry 引用
  （`status` 仍不动）？
- `allowed_paths`: `bridge/src/main/java/**`（hello 常量来源与 `Expected` 装配）、`bridge/src/test/java/**`、
...
- `counterexamples`: 运行时拿不到 mod container → 失败关闭而非回落常量；声明与 plan 不符 → 拒；
  未重建就改引用 → 拒。
- `validation_class`: `LOCAL_THEN_REAL_RUN`——静态门 + 1.21.4 侧一次握手真跑与四读一致，外加引用换代后

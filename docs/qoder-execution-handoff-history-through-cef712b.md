# Qoder 临时执行交接（历史归档，含 cef712b 后未提交的审计草稿）
...
>
> **同晚按同一规则又核了七类现在时声明（前六类落在 `1674bfb`、`7e469ce`、`f2116b0 → e4bcc1c → 030d406 → 6003ed4`
> 与其后各格，第七类的判据读数落在 `cef712b`；七格全部 docs-only，计划仍暂无 `NEXT`）**：①**本地 `main` 指针**——本节第一条命令读到 `0 104`，之后确立为
...
> 三格各两个 `CI` run 在 job 与 step 层全 `completed / success`。④**文档里每一条路径引用还指不指得到东西**
> （`.tmp/check_doc_paths.py`，与 ③ 同一套"只走 `git ls-files`"的作用域口径，纯静态）：82 份文档 →
> **689 处**去重引用 → **`findings: 0` / `exit 0`**；60 处按机器判的六类理由豁免（上游树 47、runner 里的绝对路径 4、
> 改名箭头的左半边 3、文档自己写明"不建 / 仍不存在"4、省略号占位 1、裸 basename 简写 1）。**十道变异与对照读数**
> ＝七道红各只出 1 条具名 finding、三道绿是对照组（含"真扫描本身"那一道）。这一轮又抓到守卫自己的洞：作用域曾写成
> 遍历文件系统（2635 份 md / 1037 条 finding，把 worktree 与 `.tmp` 下的克隆都扫进来了）、歧义的同尾模块路径被当成
> 不存在（**那 3 条不是文档写错，是判据太窄**）、简写规则太宽让反证直接变绿；并且**第三次**出现"记录形状本身把门弄红"
> （把变异输入写成带目录前缀的完整路径，扫描立刻读到 `MISSING_FILE`）——**依旧没有给校验加豁免名单**。
> 九道门与 pytest 在最后一格 docs 改动之后重跑，仍 `2501 passed / 2 skipped`。⑤**文档写下的卡 id / case id
> 指不指得到真注册表**（`.tmp/check_doc_card_ids.py`）：同一 82 份文档 → **186 处** id 引用 → **`findings: 0` /
> `exit 0`**；注册面读法是不认口头的那一条——只有标题行、带状态字的表格行、或非 markdown 文件里的出现才算锚点，
> "丢前缀的简写"要有**唯一**后缀才算数。2 处豁免正是上一轮被更正的那个拼造 id 自己（±1 行内文档明写它不存在）。
> **七道变异与对照读数**＝三道红各 1 条具名 finding、三道绿对照、第七道真扫描。这一轮又抓到三次自己的口径错，
> 其中最像"假绿"的一道是**反证脚本把地板设成 0，于是"作用域塌成空文档"那道永远不响**——改完每道各带地板才红在
> 命名理由上。执行文档里那句"上一格记录提交未再跑九道门"仍按同一口径给出，本轮九道门与 pytest 的读数见 todo 同名一节。
> ⑥**文档抄下的证据摘要（`run_id` / `bundle_digest`）配不配得上 registry**（`.tmp/check_doc_evidence.py`）：
> 全部 86 份 markdown（比 ④⑤ 那句 82 份多 4 份仓库其它 README）× registry **18 行 evidence** →
> **`paired citations: 15` / `distinct cited cases: 6 / 18` / `findings: 0` / `exit 0`**；归属按 **case 集合**判，
> 因为 `bridge_digest`/`launch_plan_digest` 在同一 entry 内共享，按单值判会把自己写的合法句子判成假红（这一版就撞上过一次，
> 修的是判据、不是加豁免名单）。**十道变异＝四道红各 1 条具名 finding（三种跨 case/跨 entry 错配 + 一道
> `SCAN_TOO_NARROW`）、六道绿（五道故意保留的边界：registry 里没有的 hex、裸 hex 不配 case、`attempt` 数字、
> entry 级不绑 case 的 recipe 摘要、同 entry 共享摘要，加真扫描 positive control）**。**这一格撞到的是真规格缺口**：
> 与 case 同行的 hex 有 **59 处（40 枚不同）在 registry 里根本没有**，它们是文档如实记下的被取代 attempt 与只作诊断的运行，
> 只在外部 runner 数据卷 `/data` 与本机未跟踪的 `.tmp` 运行日志里；一律判红就得到 59 条假红。**这一条第一版写重了，
> 已更正**：registry 那 18 行的字节面**不是缺口**——被跟踪的 `tools/verify_tested_provenance.py` 就是那道门（按 run id
> 在数据根里对着 bundle 自己的 manifest 复核，执行文档记着它 `verified: true` / `exit 0` 的读数，本轮没在这台机器上重跑）；
> 真剩下的缺口只是那 40 枚「被取代 attempt / 只作诊断」的 run 引用两侧都不判。按「发现规格缺口时停止并报告」
> 不新建卡也不扩范围。另外读到 registry 覆盖不对称：1.20.1 那 6 行的摘要在文档里出现
> 2/3/2/4/2/15 次，1.21.4 那 12 行一次都没出现——判它算不算缺陷属于证据面与 V08 主线，**归主控**。
> 还有**第三件只报告不代办**的事：③④⑤⑥ 这四把守卫（含反证脚本）全在 `.gitignore` 第 10 行排除的 `.tmp/` 里，
> `git ls-files .tmp` 读 0 行——这些 `rc 0` 只在留着它们的机器上可复现。把守卫提进被跟踪的 `tools/`、接进九道门与
> CI 是**新范围**，等主控点头再开卡。
> **⑦ 同一规则下的第七类：散文里写下的"现在是什么状态"本身（落在 `cef712b`）**——前六格判工件与引用，这一格判两段散文：
> 计划那份的 `## 计划元数据` 段（其第 8–375 行）加本文件全文。六条句式，每条钉一个
> 可现算的不变量：唯一规划缺口对 `tools/report_cases.py` 的 `planning_gaps`、卡片自己的 `status` 行（不读散文）、
> `git ls-files` 判"某夹具还没被创建"、`.tmp/count_plan_cards.py` 现数的 `NEXT` 条数、`local-only ∩ missing` 是否为空、
> 以及 local-only 那批 required 的条数与逐条 `present`（名单若写了就比集合）。实读
> （`.tmp/check_plan_state.py`）＝ **`sections: 2 / claims judged: 10 / data-root claims left unjudged: 11 / findings: 0 /
> exit 0`**，逐族 ABSENCE 2、CARD_STATUS 2、GAP 1、LOCAL_GAP 1、LOCAL_LIST 2、NEXT 2，十条锚点落在计划 21/26/26/37/39/42
> 与本文 10/24/24/37。**十一道变异与对照**（`.tmp/reverse_plan_state.py`，「bad cases: 0」）＝ R1–R9 九道红，各红在自己
> 命名的理由上（改面名、改卡面状态、把"未创建"挪到仓里真有的夹具、只翻卡片清点副本不动句子、只翻机器读数、只少写一个
> 名单名字、六类句式全拆 → `CLAIM_NOT_FOUND` 七条加 `SCAN_TOO_NARROW`、只拆一句 → 精确报缺那一族、`FLOOR` 只调高）；
> R10 是**不滥报的对照**（翻一个 runtime-required case 的 `present` ⇒ 十条仍全绿，判据认得 `validation_class`）、
> R11 positive control（真扫描 + 现算 `uv run --frozen python tools/report_cases.py`，与缓存基线逐字相同）、R0 证明
> 反证脚本复刻的作用域与守卫默认作用域**逐字等价**。**这一轮又抓到两处自己的判据错**：一是唯一规划缺口那条句式的面名
> 先写成 `[A-Z]+`，真值恰好全字母，换成带数字的面名才暴露它原本根本读不到（放宽为 `[A-Z0-9+/_-]+`，改判据不是加豁免）；
> 二是复刻作用域时把两段并成一段，"每段只判第一条 `NEXT`"少判一条，被 R0 那道逐字对照当场抓住（改用 `<!-- SECTION -->`
> 分段）。**按设计留下的边界**：钉了 SHA 或日期的读数是历史快照不追溯判；需要数据根 `/data` 的 11 条 `promotable` 散文
> 只计数不判；「暂无 NEXT」在两段里共 14 次（计划 5 / 本文 9），只判每段第一条那个当前锚点，其余 12 次都在写了日期或
> 提交号的历史句里；本文那条 list-less 的读数只判条数与逐条 `present`。本轮**零产品改动、零卡片状态改动、没提升任何一道门**，
> 七道 `local-only` 仍全在位、HOST 卡面状态与那份夹具的存在性一字未动。
> **归主控的仍然一样**：§5 三格所有权冻结、V08 提升、连接用户的远程服、那三张 `BLOCKED_DECISION` 卡的状态；
> 外加一条现在变成**五把**文档守卫（③④⑤⑥⑦ 及其反证脚本）全在 `.gitignore` 排除的 `.tmp/` 里——要不要提进被跟踪的
> `tools/` 并接进九道门与 CI，等主控点头，不自行开卡。
>

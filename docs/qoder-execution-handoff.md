...
>
> **同晚按同一规则又核了四类现在时声明（落在 `1674bfb`、`7e469ce`、`f2116b0 → e4bcc1c → 030d406 → 6003ed4`
...
> **688 处**去重引用 → **`findings: 0` / `exit 0`**；60 处按机器判的六类理由豁免（上游树 47、runner 里的绝对路径 4、
...
> 九道门与 pytest 在最后一格 docs 改动之后重跑，仍 `2501 passed / 2 skipped`。

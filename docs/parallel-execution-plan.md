...
| H 测试 Harness | `test-orchestrator/runner/**`、`tools/run_controlled_server.py`、对应 runner 契约测试；`seal_run_evidence.py` 需与 E 协调独占窗口。**当前施工支（2026-09-27 第五轮更新）**：`codex/minekin-auto-path-status-wiring` @ `../minekin-wt-h3`（base `6ce9f06`，卡 H3）**已合完**（合并 `cd7664b`），该 lane 手上没有活动施工支；`lane_next` = **`H1c`**（原 `QUEUED`，现前序已清，派工等 E 释放规范卷写入窗口） | 产品 `src/**`、`bridge*/**`、case 判据、registry、规范卷、主计划 | `V1201-AUTO-PATH-RUNNER-001`（G1/G2）；**等 E 不再改/依赖该批 runner 文件后才开工** |

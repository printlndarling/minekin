...
| H 测试 Harness | `test-orchestrator/runner/**`、`tools/run_controlled_server.py`、对应 runner 契约测试；`seal_run_evidence.py` 需与 E 协调独占窗口。**当前施工支（2026-09-27 第七轮更新）**：`codex/minekin-client-env-readout`（卡 `H1c`，base `9ac99c0`，lane 尖 `de579ad`）**已合完**（合并 `3706524`，远端 SHA 已核）。**第八轮更新**：该 lane 当前施工支：**无（第十一轮更新）**——上一支 `codex/minekin-h1d-run-rc` @ `../minekin-wt-h1d`（卡 `H1d`，base `796316a`）**已闭环**：lane 单笔 `21e0b6c`（三文件 `+409/-46`），M 双审后以合并 `e3ca1b7` 入 `main` 并核远端 SHA。此前各支（`codex/minekin-harness`、`codex/minekin-runner-named-failure`、`codex/minekin-auto-path-runner`、`codex/minekin-controlled-server-status`、`codex/minekin-auto-path-status-wiring`）**均已合完** | 产品 `src/**`、`bridge*/**`、case 判据、registry、规范卷、主计划 | `H1d`（V2 读数暴露的 `domain.sh` rc=0 可见性缺口；第七轮提升为 H 的 `lane_next`，**第十一轮已闭环：lane `21e0b6c` → M 合并 `e3ca1b7`**）——H lane 现无 `lane_next`，等 M 另立。第七轮那个「引擎对 `/_ping` 回 500」的阻因已于 2026-09-27 第八轮由用户启动 Docker Desktop 解除，M 实读：`docker version` → server `29.5.3`、镜像 `minekin-runner:local` id `b67a4d917306`、规范卷 `:ro` 写试探仍回 `OSError: [Errno 30] Read-only file system` |
...
| `B1-b` **第一阶段**（已闭环：E `b9c40a7` → M 合并 `3a993e0`）（`P0-OFFLINE-090-100-EVIDENCE-CHECK-001` 的仓库侧） | 甲类两条（`OFFLINE-090`/`OFFLINE-100`）缺的是 **case 定义与断言**，不是只差运行。本阶段只把「这两条判据能否逐字落在**已封存工件**的字段名上」变成仓库里可判定的形状，并按 §3.3 同名行的验收 ③ 报门载荷的前后与逐项差异 | 新增 `tests/fixtures/cases/**` 里这两条的定义与覆盖它们的测试 + 本卡自己那份记录；**不碰** `test-orchestrator/**`（第七轮注：`H1c` 已并入 `3706524`，规范卷写窗自那时起空闲）、`tools/**`（M 的面）、`src/**`、判据/registry 的 `status/gaps` | **E lane**；派工支 `codex/minekin-offline-090-100` @ `../minekin-wt-b1b`，base `03c1d95`（= 当时远端 `main`）。**阶段边界（M 第七轮定标）**：规范卷全程 `:ro`、不建 attempt/bundle ⇒ 与 `H1c` 的读卡并行**不违反**卷写窗串行；**真实封存仍等 `H1c` 合并后 E 拿回写窗**。注册这两条必然让门载荷离开 `fb0152c8…`（验收 ③(ii)），故本阶段的门读数只作「前后两枚摘要 + 逐项差异 + 各落入哪个 block」报告，`promotable` 冻结在 `W00/W10/W20/W60`；push 后 M 审 |
| `H1d` auto run 停在具名前沿时 `domain.sh` 仍 `rc=0`（已闭环：H `21e0b6c` → M 合并 `e3ca1b7`） | V2 的绿读数（M 在自己卷上重放确认）：跨过早停后停在 `BUDGET_UNDECLARED` 的 auto run 与「起了客户端但没到 playable」在退出码上不可区分（同驱动红形状给 `rc=2` ⇒ 非恒零） | `test-orchestrator/runner/domain.sh`、`tests/contract/test_runner_scripts.py`、新增 `docs/validation/**` 记录 | **H 独占面，与 `H1c` 同面 ⇒ 已排在其后**；状态 **已闭环（第十一轮，M 合并 `e3ca1b7`；派工史：第七轮提升为 `lane_next`，第八轮派工：支 `codex/minekin-h1d-run-rc` @ `../minekin-wt-h1d`，base `796316a`）**，主计划 §4 同名行有先红后绿验收。**第七轮追加一项验收（来自 `H1c` 的 M 审查发现）**：把 `launch` 深度读数取自**真起客户端那一次** wrapper 调用（或由同一条命令行先写读数再 `exec` 客户端），并对「探针屏／客户端屏」「探针 `XAUTHORITY`／客户端 `XAUTHORITY`」给成对实测对照，使记录里那句「客户端 JVM 拿到的正是后者」可判真伪。这两项都要活体容器测量——第七轮的引擎 500 已在第八轮解除，测量可开工。禁挂规范卷、禁改判据/registry/门禁/seal schema |
...
登记只解决文件面归属，**不把 lane 分支上的实现升格为主干状态**：本表中标着「已闭环」的十二行（`INT-EVIDENCE-INTEGRITY-001`、`H1a`、`H1b`、`S2`、`E-CO`、`V1201-AUTO-PATH-RUNNER-001`、`INT-REPO-CHECK-TOOLCHAIN-PROVENANCE-001`、`E-RR`、`V1201-CONTROLLED-SERVER-STATUS-KNOB-001`（H2）、`V1201-AUTO-PATH-STATUS-WIRING-001`（H3）、`V1201-AUTO-STATUS-ON-1201-001`（V2）、`H1c`）都已随 M 的合入闭环，主干 SHA 逐笔在行内；其中 `E-RR` 与 V2 同样是**按 `BLOCKED_HARNESS` 收口**的闭环。**本表此刻零行「在工」（第十一轮更新：`B1-b` 第一阶段与 `H1d` 都已闭环，读数在各自行内；E lane 回到 B2 的剩余范围，新立的 `P0-OFFLINE-100-A-B-A-RUN-001` 为 `QUEUED`（前置=M 的 `P0-OFFLINE-090-100-REGISTRATION-001` 落地）**：① E lane 的 `B1-b` **第一阶段**（第七轮派工，支 `codex/minekin-offline-090-100` @ `../minekin-wt-b1b`，base `03c1d95`，全程只 `:ro` 读规范卷、不建 attempt/bundle ⇒ 不占卷写窗）；② H lane 的 `H1d`（第八轮派工，支 `codex/minekin-h1d-run-rc` @ `../minekin-wt-h1d`，base `796316a`，禁挂规范卷）。③ M 自己的 `INT-ORCHESTRATOR-REVISION-VISIBILITY-001` **已闭环**（第十轮合并 `6495356`，远端 SHA 已核）。① 补第十轮复审结论：落位修正 `a2a93ea` 合格，但草稿两处判据设计不合主控钉下的边界（090 的凭据字面量来源与 PASS/登记形状、100 的 A→B→A 三段证法），**已退回具体两处修改**、暂不合入。`H1c` 已随合并 `3706524` 闭环（第六轮派工、第七轮收口：该 lane 首个施工会话在子代理轮次上限处被掐断、工作树留下**未提交**实现，M 在同一工作树派续跑会话**收尾而非重设计**——复量红/绿/对照/反向、写记录、两个提交、push）。`H1d` 于第七轮从候补行提升为 H 的 `lane_next`，与 `H1c` 同面且排其后，并带上 `H1c` 审查新增的那条验收（`launch` 读数须取自真起客户端那次 wrapper 调用）；它的活体测量在第七轮曾与 V lane 的下一张卡一同阻于受控容器引擎 500，第八轮引擎恢复后即派工。V lane 收口后没有下一张安全卡（1.20.1 侧缺加入者起跳面 ⇒ 需先有 H 面 runner 卡，即 `H1d`；H-3/H-4 属主控决策）。E lane 回到 B2 的剩余范围（其 LAN 形状被 ⑤ 具名阻断，见主计划 §2 分类表；M 在第六轮判断「先做 `H1c` 的环境读数、不做第三次裸重跑」），`lane_next` = `B1-b`（**第一阶段已闭环：E `b9c40a7` → M 合并 `3a993e0`**；第七轮定标：只读的仓库侧形状不必等 `H1c` 合并——`H1c` 已并入 `3706524` ⇒ **规范卷写窗自第七轮起空闲**，「真实封存等写窗」的前置已满足，等第一阶段收口即可派封存侧）。**第七轮实测的全 lane 共同阻因（第八轮已解除）**：`docker version` 对 `/_ping` 回 500，凡需挂卷或活体 JVM 的测量、对照与封存在引擎恢复前无法开工；重启用户机上的引擎进程属进程接管，留主控。第八轮 2026-09-27 由用户本人启动 Docker Desktop 后解除，M 随即在合并树 `796316a` 上补完所欠的 `:ro` 门载荷与台账复量（读数见交接文档第八轮）。

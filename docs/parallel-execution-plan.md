...
| `H1d` auto run 停在具名前沿时 `domain.sh` 仍 `rc=0` | V2 的绿读数（M 在自己卷上重放确认）：跨过早停后停在 `BUDGET_UNDECLARED` 的 auto run 与「起了客户端但没到 playable」在退出码上不可区分（同驱动红形状给 `rc=2` ⇒ 非恒零） | `test-orchestrator/runner/domain.sh`、`tests/contract/test_runner_scripts.py`、新增 `docs/validation/**` 记录 | **H 独占面，与 `H1c` 同面 ⇒ 严格排在其后**；状态 `QUEUED_PROPOSED`（主计划 §4 同名行有先红后绿验收），等 `H1c` 合并后由 M 提升为 `lane_next`。禁挂规范卷、禁改判据/registry/门禁/seal schema |

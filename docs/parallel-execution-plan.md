...
| H 测试 Harness | `test-orchestrator/runner/**`、`tools/run_controlled_server.py`、对应 runner 契约测试；`seal_run_evidence.py` 需与 E 协调独占窗口。**当前施工支（2026-09-27 第七轮更新）**：`codex/minekin-client-env-readout`（卡 `H1c`，base `9ac99c0`，lane 尖 `de579ad`）**已合完**（合并 `3706524`，远端 SHA 已核）。**第八轮更新**：该 lane 当前施工支 = `codex/minekin-h1d-run-rc` @ `../minekin-wt-h1d`（卡 `H1d`，base `796316a`），在工、尚无提交。此前各支（`codex/minekin-harness`、`codex/minekin-runner-named-failure`、`codex/minekin-auto-path-runner`、`codex/minekin-controlled-server-status`、`codex/minekin-auto-path-status-wiring`）**均已合完** | 产品 `src/**`、`bridge*/**`、case 判据、registry、规范卷、主计划 | `H1d`（V2 读数暴露的 `domain.sh` rc=0 可见性缺口；第七轮提升为 H 的 `lane_next`，**第八轮已派工在工**）。第七轮那个「引擎对 `/_ping` 回 500」的阻因已于 2026-09-27 第八轮由用户启动 Docker Desktop 解除，M 实读：`docker version` → server `29.5.3`、镜像 `minekin-runner:local` id `b67a4d917306`、规范卷 `:ro` 写试探仍回 `OSError: [Errno 30] Read-only file system` |

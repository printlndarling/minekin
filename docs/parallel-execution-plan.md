...
登记只解决文件面归属，**不把 lane 分支上的实现升格为主干状态**：本表中标着「已闭环」的十一行（`INT-EVIDENCE-INTEGRITY-001`、`H1a`、`H1b`、`S2`、`E-CO`、`V1201-AUTO-PATH-RUNNER-001`、`INT-REPO-CHECK-TOOLCHAIN-PROVENANCE-001`、`E-RR`、`V1201-CONTROLLED-SERVER-STATUS-KNOB-001`（H2）、`V1201-AUTO-PATH-STATUS-WIRING-001`（H3）、`V1201-AUTO-STATUS-ON-1201-001`（V2））都已随 M 的合入闭环，主干 SHA 逐笔在行内；其中 `E-RR` 是**按 `BLOCKED_HARNESS` 收口**的闭环（记录入了主干、卡的目标即当前构建 LAN PASS 未达成），V2 同样是**按 `BLOCKED_HARNESS` 收口**的闭环（它补上了 1.20.1 × `--enable-status` 那格，但端到端 auto JOIN 仍阻于 bridge 工件）。**本表此刻只有一行「在工」**：H lane 的 `H1c`（第六轮派工，支 `codex/minekin-client-env-readout` @ `../minekin-wt-client-env`，base `9ac99c0`，规范卷只 `:ro` 且不封存）；`V2` 已随合并 `b6f23a8` 闭环，两张的面本就不重叠（`H1c` 改 `test-orchestrator/runner/**` 与其契约测试，`V2` 只加自己那份 `docs/validation/` 记录并只读仓库）。V lane 收口后**没有下一张安全卡**（1.20.1 侧缺加入者起跳面，需先有 H 面 runner 卡；H-3/H-4 属主控决策）。E lane 回到 B2 的剩余范围（其 LAN 形状被 ⑤ 具名阻断，见主计划 §2 分类表；M 判断先做 `H1c` 的环境读数、不做第三次裸重跑），`lane_next` = `B1-b`（`QUEUED`，等 `H1c` 合并后派工）。

...
3. **运行时路由门禁**：每个 DTO带 `information_class=PLAYER_EQUIVALENT|MANAGEMENT_ONLY|TEST_ORACLE`；只有第一类能进入人物认知路径。（2026-09-22：**这一道已实现**，`domain/information_class.py` 管类别表与准入判决，`cli/session_runtime.py` 在每个事件上问它并把结果记进 run document，用例是 `HOSTCTL-070`，四条断言。**有一处是照性质、不是照字面做的，理由必须写在这里**：契约说 DTO **带** `information_class`，最直白的读法是加一个字段——但字段由**发送方**设置，而这条用例的前提恰恰是发送方（host-control）不可信：一个由管理侧给自己打分的类别不是控制，是注释。所以类别住在**读方**的表里，按消息类型查，表里没有的类型**拒绝**而不是默认放行；这张表与 IPC 层那份入站事件表由用例**双向绑定**（多一个入站 DTO 而没给它定类别就红，删了消息却留着类别也红），因此「每个 DTO 都有类别」是判据而不是口号——**如果将来真需要让线缆带上这个字段**（给仓库外的消费者、给 Dashboard），它必须**与这张表对照检查**、绝不能被相信，和 `trust_class` 遵守同一条规则。**这道门禁不能按"payload 长得像什么"判**，这不是哲学问题：protobuf 不是自描述的，`HostLifecycle` 的字节按 `InitialObservation` 解出来是能过的，`game_tick` 读回的是 generation（两条消息的 2 号字段都是 varint）——用例把这一对**一起断言**，因为那正是形状不可用作判据的原因。四个入站 DTO 的类别：`InitialObservation`、`ConnectionLifecycle`、`ActionResult` 是 `PLAYER_EQUIVALENT`（快照、自己的连接相位、自己动作的下场，都不含世界状态），`HostLifecycle` 是 `MANAGEMENT_ONLY`（它是控制面关于"这把世界发布出去"的机械报告，不是 Kin 所在的世界的知识）。**仍未做的是这条的真实一半**：真客户端发出的这些事件、以及 `HOST-001…100` 一条都还没有定义，所以 `HOSTCTL-070` 的 `mandatory` 仍是 `false`，理由与上面 `HOSTCTL-001`/`010` 那两条相同。）
...
8. `HOSTCTL-070`：host-control尝试输出 server entity/player/inventory DTO；schema/路由门禁拒绝。（2026-09-22：**已定义、有实现、跑得动**，`tests/fixtures/cases/hostctl-070.json`。四条断言：类别表与 IPC 层入站事件表**双向绑定**、管理类 DTO 被拒、未分类/服务端 DTO 名被拒、以及唯一穿过真实 IPC 环回的那条（管理侧的报告进了 run document，却没进认知路径——"门禁存在"与"运行时真的问它"是两句话，这条用例断言的是第二句）。它自己的核心断言把**字节**与**路由**配成一对：同一份字节按 `InitialObservation` 解得出、按它自己的类型路由被拒。`mandatory` 仍是 `false`，理由同上。）

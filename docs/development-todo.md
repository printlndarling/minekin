...
- [ ] 但这条策略**还没有在真实启动路径上被应用**：§13 的启动顺序要求第 4、5 步（把未正常结束的 run/session/outbox 标记为待对账、使历史 lease 与 generation 失效）在 `session start` 之前跑一次，而现在只是有了那张表和它的判据，没有从 store 读出未决 outbox 并据以行动的服务。`domain/recovery.PendingEffect` 是结构化的 Protocol，正是为了让那一步能用 port 自己的 `OutboxItem` 而不必反向依赖。

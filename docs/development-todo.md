...
- [ ] **E lane** `NEXT`：`P0-CONTROLLED-CAMPAIGN-001`（主干 `NEXT` 是 M 的 `PARALLEL-INTEGRATION-GATE-001`，本卡在其 §2 激活表里归 E 独立分支施工）。B 段第二张：在受控 dedicated offline 与必要 LAN 场景补当前 build 的 mandatory sealed bundle。**第一刀（`CORE-040`、`CORE-050`）与第二刀（`OFFLINE-030`）已完成，卡片仍在进行**；剩余是 5 条只有别的构建 bundle 的 `ADMIT-001/040/060/100/110`（**这 5 条与 `OFFLINE-030` 一样都不是 mandatory，跑完不动任何门**），以及卡面后半段 L3/L5/L6、崩溃恢复、重启协调、offline identity 与 soak。今天 11 个包的全部 block 字段里没有 `CASE_VERSION_MISMATCH`，剩余阻因是 `REQUIRED_CASE_NOT_REGISTERED` / `NO_MANDATORY_CASES`（主控）。

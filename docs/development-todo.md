...
- [ ] 为受控 Linux runner 准备 Java 21、Xvfb、原版 1.21.4 server 与隔离账号/目录。**本机已用 Docker 把前半段跑通并量出真正的拦路石**：`eclipse-temurin:21-jdk-noble` + `apt install python3-venv xvfb` 就能同时提供 Java 21.0.12、Python 3.12.3 与 Xvfb；把仓库只读挂到 `/src`、`PYTHONPATH=/src/src`、从仓库根运行 `python -m minekin_core`（**不能**用装好的 wheel——`find_workspace_root` 需要同时看见 `bridge/` 与 `proto/`，装进 site-packages 就看不见了，这一点文档里早有记录），数据根放 Docker volume。这样跑 `doctor`：**python/java/protobuf 三项通过，只有 sqlite 不通过**。**拦路石是 SQLite 版本**：冻结的 WAL 安全门禁要求 >=3.51.3 或回补版 3.50.7 / 3.44.6（出处是 SQLite 官方 WAL 页 2026-08-25 的 WAL-reset 修复说明），而实测各基础镜像里 Python 自带的 SQLite 是：Ubuntu 24.04 → 3.45.1、Debian trixie → 3.46.1、**Alpine → 3.53.4**。也就是说常见发行版基础镜像**都过不了这道门禁**，只有 Alpine 这类滚动发行版可以；而 Alpine 是 musl，Minecraft 的 LWJGL native 是 glibc 构建，所以「SQLite 够新」与「客户端 native 能加载」两个要求目前指向不同的基础镜像——这是 runner 必须先定的一个取舍，不是可以顺手选的。

...
- [ ] 同一个契约还要求 `artifacts.mods: ["<artifact,digest,license>"]`，而计划里只有 `fixed_mods` 这两个**名字**：fabric-api 的 jar 被 recipe 校验过却从未被放到客户端会加载的位置，Bridge 的 jar 同理。因此 `launchable: true` 现在意味着"这份计划自洽"，**不**意味着启动起来的客户端有 Bridge——它会以原版状态起来，然后握手永远不来。这是下一批：把固定 mod 集物化进 overlay 的 `mods/`（fabric-api 取自 artifact store，Bridge 取自被 pin 的构建产物），并让启动前的就绪检查覆盖它们。

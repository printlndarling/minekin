...
- [ ] 运行时**还没有被 `session start` 调用**：现在唯一的调用方是契约测试（真实 host + 真实分类器 + 真实会话状态机 + 一个扮演客户端的 loopback 对端）。接线还差三件具体的事：`session start` 要在启动客户端**之前**建 host 并独占写出 descriptor（overlay 里已有 `ipc/` 目录）、把 `MINEKIN_BRIDGE_DESCRIPTOR` 传下去、并把启动路径改成同一个事件循环里 prepare→spawn→supervise——host 的监听 socket 绑定在创建它的 loop 上，因此分成两次 `asyncio.run` 会直接把 transport 留在已关闭的循环里。在那之前不给客户端传这个变量：传了只会让它从一个不存在的文件启动，比现在明确的"未配置"报错更糟。

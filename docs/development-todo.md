...
- [ ] 这个适配器**还没有调用方**：没有运行时循环去 `receive_event`，也没有把它接到会话状态上。`tests/contract/test_bridge_ipc_host.py` 里那条端到端用例证明的是「真实 host + 真实分类器」这条链成立（字节从 loopback event channel 出来、经 `apply_lifecycle` 走到 `PLAYABLE` 再走到 `DISCONNECTED`），不是「真客户端会这么发」。已做变异验证：把 `DISCONNECTED` 映射到别的信号，单元与契约用例同时失败。

...
- [ ] 同一次扫描里还剩一条**已确认但未修**：`kin/` 下若放着一个**不是 Minekin 账本**的 SQLite 文件（application_id/表不符），读路径会在 `no such table: kin_identity` 上以驱动异常失败并被脱敏。`connection.py` 已有 `_validate_schema`（校验 application_id 与期望表）但它只在写路径的 `migrate` 里被调用；读路径既没有它、也不能简单地把它挂进 `connect_reader`（未迁移的库也该能被读吗？这需要先定）。**注意**这个洞与上一条不同：上一条只是「文件不存在」，这一条需要区分「文件不是我们的」与「文件被锁住」——把 `OperationalError` 一律读成前者会是新的误报。

...
- [ ] `NEXT`：`P0-CONTROLLED-CAMPAIGN-001`。B 段第二张：在受控 dedicated offline 与必要 LAN 场景补当前 build 的 mandatory sealed bundle。B1 给的最小内容是先补 `CORE-040`、`CORE-050`（唯二只差运行的 mandatory 行），再按判据形状处理 `OFFLINE-030` 与那 5 条只有别的构建 bundle 的 ADMIT 行。
- [ ] `QUEUED_PROPOSED`（B1 交出，等主控排期）：`P0-CORE-040-050-RUN-001`（两条 mandatory 的当前构建真跑）、`P0-OFFLINE-090-100-EVIDENCE-CHECK-001`（甲类：字节已封、缺 case 定义与断言；Dashboard 那半句要登记为材料外边界）。
...
- [ ] `BLOCKED_EVIDENCE`：`P0-GATE-PROMOTION-001` 与各门晋级——`promotable` 只是机器候选， mandatory 真实证据（现由 `P0-CONTROLLED-CAMPAIGN-001` 承接）补齐前不用单测或旧包替代。

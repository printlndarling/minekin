...
- [x] 完成 Gradle 依赖锁与 SHA-256 verification metadata；固定 Wrapper 8.12.1 已在 Java 21 下通过离线 strict verification `clean check`。**但这份 metadata 目前只在一台机器上成立**：它记录了 Loom 本地工件的本机摘要，换平台即 mismatch，详见 W10 两条。
- [x] 用 Docker 实测过 Linux 侧：容器里的 Linux 构建能走到 Loom remap 之后的依赖校验那一步，说明 JDK、依赖缓存与锁文件本身够用，卡住的是平台相关的产物摘要（同上）。
...
- [x] **跨平台可复现问过了，答案是不**——而且现在有可复现的证据，不再只是缺少 runner。做法是把源码只读挂进 Linux 容器、用固定 JDK 21 构建（见下方方法），失败点不是构建而是**依赖校验**：51 个 Loom **本地产出**的工件摘要与仓库里记录的不符，且是 checksum mismatch 而不是缺条目。例如 `fabric-api-base-0.4.54+b47eab6b04.jar`：记录值 `59f44704…`（Windows 产出），Linux 产出 `1d4b01ea…`；`minecraft-merged-…-v2.jar` 同理。也就是说决定最终字节的不只是 Loom 的 remap 阶段，还有**它跑在哪个平台/哪个 JDK 构建上**。两个变量这次没能分离：本机是 Windows + `21.0.12.1+1-LTS-4`，容器是 Linux + Temurin `21.0.12+8`，所以结论应表述为「这两个组合的产出不同」，而不是「操作系统是原因」。据此，**在同一个平台与同一个 JDK 构建上固定之前，JAR 摘要不得写进 recipe**，`build_required` 继续保持诚实。本机（Windows + 该 JDK）连续两次 `clean build` 仍然逐字节相同（`fc1692d2…`），所以问题在跨组合、不在单机。
...
- [x] 让下一次能直接重跑：`docker volume create minekin-gradle` 后把 `~/.gradle` 的 `caches/modules-2`、`caches/fabric-loom`、`caches/jars-9`、`wrapper` 灌进这个卷（**先 `./gradlew --stop`**，否则活锁文件会以「Unexpected lock protocol found in lock file」或读错误的形式破坏拷贝），再 `docker run --rm -v <repo>/bridge:/src:ro -v minekin-gradle:/gradle -e GRADLE_USER_HOME=/gradle -w /work eclipse-temurin:21-jdk-jammy` 里 `cp -a /src/. /work/ && rm -rf /work/build /work/.gradle` 后构建。两点经验：**不能**把 Windows 的 `~/.gradle` 直接 bind mount 进容器（Gradle 的 file-access journal 在那种挂载上直接 `Input/output error`），也**不能**把宿主已有的 `build/` 一起拷进去（会拿旧 jar 当成本次产物，得到一个假的"一致"）。
...
- [x] 接线时撞上一个真的冲突：ledger 的写入是 `asyncio.run(...)`（每次写自建一个 writer），而它现在被一个**正在运行的**事件循环调用，直接 `RuntimeError: asyncio.run() cannot be called from a running event loop`。改法是把整个启动（spawn + marker + 两条事件）放到 `asyncio.to_thread` 里执行：既是阻塞工作，又让 SQLite 的 fsync 不落在事件循环上——§9 本来就要求这一点。**仍未修的是根因**：ledger 这个"同步包一层 asyncio"的形状一旦从循环里调用就会炸，现在只是被推到了线程里；等有第二个从循环内写账本的调用方时应当给它一个真正的 async 入口。

...
- [x] 因此 `build_required` 的含义收敛成了另一件事：**跨平台可复现已证明成立**（W10 末条的四条证据），所以钉住的不再是「Loom 1.9.2 的行为」；真正还没做的是「一个**本地构建**产物如何成为受校验的 bundle 成员」这个 schema 决定。在那之前 `build_required` 仍是诚实的状态——但它同时是 `require_launchable` 的硬阻断，因此它是启动路径上真正的关口。
...
- [ ] 下一步由此明确：既然摘要可复现，就应该**把 JAR 摘要钉进 recipe**、让 `build_required` 变成真实 pin，从而解除 `require_launchable` 对整条启动路径的硬阻断。这不是改一个字段：Bridge 是**本地构建**而非下载的工件，所以要先定「一个本地构建产物如何成为受校验的 bundle 成员」——摘要放在哪、已构建的 JAR 从哪个路径取、`require_store_complete` 怎么认它。这是 schema 级决定，单独一批做。

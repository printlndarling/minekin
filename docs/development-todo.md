...
- [ ] **仍未做的、也是它与"能启动"之间的真正距离：没有任何东西把固定 mod 集放进客户端的 mods 目录。** `plan["fixed_mods"]` 只是两个名字；fabric-api 的 jar 虽被 recipe 校验却从未被放到客户端会加载的位置，Bridge 的 jar 同理。所以 `launchable: true` 现在意味着"这份计划自洽"，**不**意味着启动起来的客户端会有 Bridge——它会以原版状态起来，然后握手永远不来。这是下一批：把 mod 集物化进会话 run 目录（fabric-api 取自 artifact store，Bridge 取自被 pin 的构建产物），并让启动前的就绪检查覆盖它们。
...
- [ ] 但**这条链目前仍到不了终点，缺口比 mods 更大：没有任何东西在抓取工件。** `ArtifactFetcher` 是一个有测试、没有调用方的库，而冻结的 CLI 表面里也没有任何命令驱动它——于是 4,120 个下载工件与 fabric-api 都无法进入 store，`install_fixed_mods` 会以"not in the content-addressed store yet"拒绝。这不是可以顺手补的一行：谁来抓、什么时候抓（启动前显式抓，还是 `session start` 里抓）、以及冻结 CLI 是否要新增一个命令，都是决定。在那之前，`session start` 走到了 mods 这一步就会停下，而不是悄悄启动一个没有 Bridge 的客户端。

...
6. `CORE-050`：握手前、JOIN前、首快照前的输入全部被拒绝。（2026-09-20 状态：Core 侧现在已经**可以测**了——一次被拒绝的请求是一条账本事件（`InputRefused`，带 phase 与仲裁器自己的理由），而不是 run document 上会被后一次覆盖的那个字符串；`session start --hold-at join` 让一次运行**故意问得太早**。于是「JOIN 与首快照之前不授 lease」这条不变量第一次可以被违反，也第一次有证据：单元/契约层已实测（问在 join 上 → `InputRefused{phase: JOIN_SEEN, refusals: [NOT_PLAYABLE]}`、没有任何 `InputLeaseGranted`、运行不因此结束；默认不问 → 一条拒绝记录都没有），而**握手指令之前**那一段由传输层拒绝所有控制命令（含输入）覆盖。尚未做的是用例文件与那次真实运行，因此 CORE-050 现在还没有 fixture，也还没有进入晋级门禁。）

...
| client JVM 强杀 | `CORE-060-CLIENT-001` | 同上，目标 `client_jvm`，账本记 session 结束 | 可（`MINEKIN_DOMAIN_KILL_CLIENT`，`domain.sh:36/1389`） | **有 1 份 `PASS`**（2026-09-24）：run `c89f5d3582e74250b27cf4a034314c0a`，`attempt_sequence: 1`、`case_version` `dc85eb043861…`、`bridge_digest` `faeec4a9df83abb9…`、`from_repository_build: true`、`re_judged: AGREES`。旧 build 那份 `PASS`（`e00f2851…`）对今天的 criteria 仍是 `UNJUDGED`，不追认也不动 |
| server JVM 强杀 | `CORE-060-SERVER-001` | 目标 `server_jvm`、`/proc` 身份消失、无 `Stopping the server` | 可（`MINEKIN_DOMAIN_KILL_SERVER`，`domain.sh:30/1316`） | **有 1 份 `PASS`**（2026-09-24）：run `f4365a50077647babda76cec90164093`，`attempt_sequence: 1`、`case_version` `50ca1ae2e22d…`、`bridge_digest` `faeec4a9df83abb9…`、`from_repository_build: true`、`re_judged: AGREES`；这一窗的 `LEFT_PLAYABLE (PLAY_ENDED)` 松键行**确实写出来了**（客户端与显示都活着，tick 路径走得通）。旧 build 那份 `PASS`（`652043a6…`）仍 `UNJUDGED` |
| 崩溃后重启重验（瞬时状态失效、世界重新观察） | `CORE-090` | 本次 run document + 同账本上一条 run 的事件行（`previous-run-trace.jsonl`）+ `recovery` 块 | 可（两连跑：先 `MINEKIN_DOMAIN_KILL_CORE=1`，紧接着 `MINEKIN_DOMAIN_CASE=CORE-090 MINEKIN_DOMAIN_STILL=1`） | **有 1 份 `PASS`**（2026-09-24，两连跑）：崩溃那次 run `0237e24c846f487ea84e8689c24a0528`（不给 case id，不封存），重启那次 run `3e94d49aace44b00924efeb1bb83c1da` / bundle `8bda01b52ae2bbfe…` / `case_version` `88fa467d8896…` / `attempt_sequence: 1` / `re_judged: AGREES`。旧 build 那份 `PASS`（`c993c801…`）仍 `UNJUDGED` |
| 正常退出 | `CORE-020` 的 `leave_after_join_observed`（也出现在 `CORE-060-CLIENT-001` 的断言集里） | 客户端日志与服务端读数 | 可 | **有 1 份 `PASS`**（2026-09-24）：run `8a72dcdf9e9c4fd190860d16a5d1bf1f` / bundle `3c4b04894069df5b…` / `case_version` `7c01d11ed1e9…`（与冻结表登记的当前 digest 逐位相同）/ `mandatory: true` / `re_judged: AGREES`；命令不带任何 kill 开关、不带 `--hold-*`、不带探针。旧 build 那几份（含一份本来就是 `FAIL` 的 `2cab1052…`）仍 `UNJUDGED`，不动不撤 |

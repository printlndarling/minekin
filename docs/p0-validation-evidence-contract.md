...
7. `CORE-060`：逐个强杀 Runtime、Launcher、client、server；验证 lease、松键、回收和重验。（2026-09-20 状态：**两种已经有真实运行**——强杀 Core（`MINEKIN_DOMAIN_KILL_CORE`，判据是账本里的 lease + 客户端日志里的 `IPC_LOST` 松键 + 服务端读数停住）与**强杀 server**（新增 `MINEKIN_DOMAIN_KILL_SERVER`，判据是服务端日志停在半句上、**没有** `Stopping the server`，客户端报告 `CONNECTION_PHASE_DISCONNECTED` 并 `released 1 input(s) after LEFT_PLAYABLE (PLAY_ENDED)`，账本记 `SessionInterrupted{phase: DISCONNECTED}`）。另外两种与「回收/重验」尚未覆盖，因此该用例仍是 **mandatory: false**。**本仓库里 Runtime 与 Launcher 是同一个进程**（CLI 既持有运行时又启动客户端），所以契约里的四种强杀在这里只有三种是不同的故障——记下来而不是假装覆盖了四种。**四次强杀无法写成一个用例的断言集**：晋级规则是「某个 case id 有任何一份满足的 bundle」，而一次运行只经历一种故障，所以不同故障的断言不可能同时成立；这一条需要「一个用例可以要求多份 bundle（每种故障一份）」的规则，那是一个**尚未做的设计决定**，写在 W70 的待办里。顺带一提，这一步的故障注入抓到了两个真缺陷（见 W70 记录）：连接 deadline 会把一个 Kin 已经在里面的世界取消掉（连同它清掉的 generation 一起，让世界的死亡变得无法上报），以及 harness 的「强杀 server」其实杀的是包装进程、JVM 活着被干净地保存了下来。) **【2026-09-20，故障注入第一阶段】** 强杀 Core 那一轮的证据里现在多了一份**故障记录**（`fault-injection.json`），而不再只靠 harness 自述「我杀了 Core」：目标进程由本 run 已持有的 pid 在 `/proc` 里**推导**（不是 `pkill -f` 扫全容器，也不是 `pgrep -P | head -1` 猜第一个子进程），角色特征必须**唯一**命中——0 个或多个一律拒绝且**不发信号**；记录 pid、`/proc/<pid>/stat` 的 starttime_ticks、pid namespace inode、exe/cmdline digest 与父进程身份；发信号前**重读身份**（TOCTOU），SIGKILL 后等待**该身份从 `/proc` 消失**（条目消失、僵尸、或同一 pid 换了 starttime 记为 PID 复用，三者可区分）。**边界**：helper 不是被杀进程的父进程，拿不到 `waitpid`，所以 `confirmation_strength` 只能是 `IDENTITY_DISAPPEARED`，**不允许**声称 `WAIT_STATUS`（schema 保留该值给未来真正是父进程的 helper，reader 今天直接拒绝声称它的记录）；外层 supervisor 的退出状态由真正 wait 了它的 runner 事后写入，server 那一路在封存时 supervisor 尚未退出，因此**明确记为未观测**。`core-060.json` 因此新增断言 `runtime_controller_sigkill_was_confirmed`，**case_version 随之变化，此前封存的 CORE-060 bundle 一律成为 legacy partial**（仍可封存与校验，但不再是该用例的证据）。强杀 server 的 `server_jvm` 记录**可以生成、可以严格校验、可以封存，并有单测**，但本阶段**不新建、也不宣称** server 用例完成。**仍未验证**：本机不能运行 Linux 容器，所以 `/proc` 的实际读取路径只有单测覆盖（注入的 procfs + 真实 stat 形状），end-to-end 未跑。**

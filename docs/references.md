...
| [Yarn 1.21.4+build.8 MinecraftServer](https://maven.fabricmc.net/docs/yarn-1.21.4%2Bbuild.8/net/minecraft/server/MinecraftServer.html) | `isOnlineMode()`说明服务器是否向 Session Service 认证；集成服务器接受未认证玩家，并暴露 `openToLan`，2026-09-16 核对 | 支持默认 LAN/offline 身份路线的代码级依据；不代表任意服务器允许未认证连接，也不解决身份冒用 |
| [Velocity player information forwarding](https://docs.papermc.io/velocity/player-information-forwarding/) | 官方项目文档说明关闭后端 `online-mode` 会停止后端自行认证，并警告不安全转发可被冒名，2026-09-16 核对 | 代理架构资料，不等于所有 offline-mode 部署；Minekin 不据此自动改服或绕过验证 |
| [Microsoft OAuth 2.0 device authorization flow](https://learn.microsoft.com/en-us/entra/identity-platform/v2-oauth2-device-code) | **可选 online-mode 适配器**候选：无本地浏览器时由用户在另一浏览器授权 | 非默认启动链；只覆盖 Microsoft 身份平台，Minecraft entitlement/profile、Xbox/Minecraft token 链、应用注册与安全存储仍需单独验证 |
...
| [Minecraft 官方账号要求](https://help.minecraft.net/hc/en-us/articles/19615552270221-Accounts-Required-to-Play-Minecraft) | 仅用于可选 online-mode 认证能力与官方服务资格的背景，2026-09-16 复核 | 不适用于把 LAN/offline-mode 默认路径强制改成 Microsoft 登录；也不保证私服兼容 |
| [Yarn 1.21.4+build.8 ClientPlayNetworkHandler](https://maven.fabricmc.net/docs/yarn-1.21.4%2Bbuild.8/net/minecraft/client/network/ClientPlayNetworkHandler.html) | `getConnection/isConnectionOpen`及断线相关正常客户端接口，2026-09-16 复核 | 接口存在不证明任一身份模式可连接、渲染或在图形服务稳定运行 |
...
| [Microsoft OAuth 2.0 device authorization grant](https://learn.microsoft.com/en-us/entra/identity-platform/v2-oauth2-device-code) | 可选在线认证资料：官方列 `/devicecode`、轮询间隔、`client_id`、access/refresh token 与 MSAL 推荐，2026-09-16 核对 | 非默认身份路径；只覆盖 Microsoft 身份平台，不证明 Minecraft entitlement/profile；应用注册、scope、续期和撤销须原型 |
| [HeadlessMc 官方仓库](https://github.com/headlesshq/headlessmc)及[MIT LICENSE](https://github.com/headlesshq/headlessmc/blob/main/LICENSE) | 命令行启动 Java 客户端、Docker、Xvfb、LWJGL stub 与版本相关优化 Mod 的现有实现证据，2026-09-16 核对 | 其 stub/dummy-assets 路线不保持 Minekin 所需的真实渲染/Live View；只能借鉴 launcher/测试思想，代码与依赖仍须逐项审查 |

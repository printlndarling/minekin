...
- **缺当前 build 的 mandatory 真跑**：§6 点名的 `CORE-040`、`CORE-050`。日志 §9 把那 28 行按判据形状切成三格，
  ...
  **后效（同日，不改上表读数）**：那 1 条「判官要运行材料且本根零 bundle」（`OFFLINE-030`）已在当天第二刀封上当前构建的
  PASS bundle（见 [OFFLINE-030 真跑封证](p0-offline-030-run-2026-09-26.md)）。该格今天为 0，但它是 `mandatory: false` 的行，
  所以本节点名的门阻因（缺 fixture / `NO_MANDATORY_CASES`）一条都没变。
  **后效续（同日，仍不改上表读数）**：§6 那 10 条 `only_another_build` 里非 deferred 的 8 条也已在当天
  第三、四刀封上当前构建 PASS bundle —— 5 条 `ADMIT` 见[第三刀记录](p0-admit-five-run-2026-09-26.md)，
  `CORE-030`（本卡唯一一条 LAN 形状）见[第四刀记录](p0-core030-lan-run-2026-09-26.md)。该格今天只剩
  deferred 的 `HOST-030/040`，即上文「9 条最小内容」已全部有当前构建 sealed bundle；`CORE-030` 同样
  `mandatory: false`，反证里补上/取走/退回三种卷状态给出同一份门读数，所以这三刀都没碰任何门的阻断。
  **后效续二（同日第五刀）**：同一格里的第三桶「判官只需仓库字节但本根零 bundle」从 **17 条** 变成 **13 条**——
  `OFFLINE-001/040/050`、`ADMIT-080` 四条由仓库自检通道（`run_repo_case.py` + `seal_repo_case.py --data-root /data`）
  封上当前构建 PASS bundle，逐条读数与反证见[第五刀记录](p0-repo-four-case-run-2026-09-26.md)。剩下的 13 条
  **全部是 HOST 家族**（`HOST-010/020/050/060/070/080`、`HOSTCOMMIT-090/110`、`HOSTCTL-001/010/050/060/070`），
  即 §5 末行所说的 deferred 族：判据在本地代码里已成立，缺的是 §5 ownership 决策而不是证据生产，故本刀不封。
  四条都是 `mandatory: false`，且封证前后两次 `report_promotion.py` 的门载荷摘要逐字节相同
  （`fb0152c85d029ee0…`）⇒ 上表读数与本节点名的门阻因一样都没有变化。

...
| Launcher / Account / Version Services | 是 | Web 授权、服务器协议探测、官方工件下载校验、Java/Fabric/Bridge 版本包和缓存 |
| Managed Client Supervisor | 是 | 在隔离工作目录与虚拟显示/隐藏窗口中启动、监控和回收真实客户端 |
| Minecraft Client Bridge | 技术适配器 | 受限观察、本地反射、输入仲裁、动作反馈、客户端心跳 |
| Minecraft Java Client | 运行时下载的受管理依赖 | 用正常账号进入服务器并渲染/执行真实玩家动作；不依赖用户已有启动器 |
...
| Launcher/Client Manager | 独立后端服务 + 内容寻址工件缓存 | 负责官方元数据、账号授权、服务器探测、Java/Fabric/Bridge bundle 与无窗口进程；不把游戏资产预装进发布镜像 |
...
| account_profile_ref | 指向 Minekin Account Broker 的授权档案，不保存明文密码或把令牌交给 LLM |
...
- 账号授权由 Minekin Account Broker 通过 Web 发起，refresh token 进入系统密钥环或加密 secrets store；Dashboard 只见 account_profile_ref，模型、Bridge、聊天和网页工具均拿不到令牌；
...
3. Account Broker 验证授权；Server Probe 获取协议/版本证据，Version Resolver 选择已验证 bundle；
...
4. 修改 host/port、正确/错误/未知协议、多版本代理、关闭 ping、未登录账号、白名单拒绝和资源包要求时，探测、bundle 选择与连接结果真实，不假报在线。
...
12. 全新 Linux 主机不安装桌面启动器，通过 Web 授权账号、添加服务器并自动准备受支持客户端；宿主无可见游戏窗口，客户端仍有真实渲染上下文和 Live View。
...
- Account Broker 的具体认证库、应用注册、Minecraft 服务链和加密 secrets backend；

...
在线认证拒绝的“不得自动启用”按[远程准入契约的 ADMIT-040 判据](p0-remote-admission-contract.md#admit-040-的可复判证据边界)
验收：Core 在连接前冻结并记录离线认证策略，策略不随服务端拒绝而重绑定；
变更认证方式必须由运行者显式改 profile、另起 run。只看失败次数或进程数
不足以证明这条约束。

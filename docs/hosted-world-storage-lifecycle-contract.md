...
（2026-09-22：**判决那一半实现了**，`domain/world_locking.py`，用例 `HOST-010` 五条断言、`run_repo_case.py` 跑出 `PASS`。三处是照契约的话做的：**（一）「两把锁都拿到」是一句话，而这个模块判它是不是真的**——创建状态机上 `PREPARED -> LOCKED` 那条边的信号是 `SUPERVISOR_AND_LOCK`，机器收下的是**说法**，这里判的是它**成不成立**（与仓库里其它地方「报告 vs 读方检查」同一条分工）；两把锁问的是**两个不同的问题**（Minekin 是否已经起了另一个会话／存档是否被别的进程打开），所以只问一把的规则只管了一半的相撞方式。**（二）租约只在确认旧主人已经不在之后才能回收**：「进程不在了」与「没人能确定它在不在」是两个发现，所以 `owner_is_running` 有**第三个值** `None`，它不是 `False` 的同义词——把 `None` 当 `False` 就是拿猜测起第二个服务端。**（三）看见一个 `session.lock` 文件不等于知道锁被持有**：契约自己把「只凭遗留的 `session.lock` 文件就能判断进程仍活着」列在**不能据此声称**那一列，也禁止见到就删，所以 vanilla 锁的状态在这里是一个**观测值**，`UNKNOWN` 是一个正经取值：没读出来的锁**拒绝启动**，而不是当成空闲。**仍未做的**：两把锁各自的**写入方**（谁写租约、谁读 vanilla 的 `isValid`）都还没有——存储的挂载策略未冻结，租约落盘属于那个还没建的 store；以及真正两个进程抢同一个世界的运行。因此 `mandatory` 仍是 `false`。）
...
4. `HOST-030`：端口正常/占用/被防火墙阻断；本地世界与 LAN结果分开。（2026-09-22：**"分开"那一半补上了**。这条用例原本就有四条**运行材料**判据（世界是哪一把、客户端是否发布在它拿到的那个端口上、Core 是否被告知），它们要一次真实运行；本轮加的是**写方**这一侧：`domain/host_publication.py` 持有"阶段与端口必须自洽"这条规则，`cli/session_runtime.py` 在把报告写进 run document 之前问它。**两点是量出来的**：**(一) `LAN_OPENED` 必须给出一个端口，而 `0` 不是**——proto 自己写着"零永远不是一个地址：读者会连上一个什么都没有的地方"，而且"说发布了却不说在哪里"**比说失败更坏，因为它看起来像成功**；**(二) `LAN_OPEN_FAILED` 不许带端口**——"没人在听"和"有东西在听"是两个断言，不是一条。Bridge 侧本来就拒绝**产生**这两种形状（`LanPublication` 与它那三条用例），但那与**读方检查**是两件事：报告走过 IPC 边界，而它会落进**加入方要读的那个文档**里；顺带把端口上界与判据工具共用了一份（`MAX_PORT`），不再有第二处拼写。`mandatory` 仍是 `false`：这条契约真正要求的"端口正常/占用/被防火墙阻断"三态要真实客户端与服务端。**仍未做的**：那三态本身，以及 `HOST-030` 原来那四条运行判据所需的运行。）
5. `HOST-040`：第二个真实客户端加入、游玩、离开、host提示后关闭。
6. `HOST-050`：保存前、保存中、保存后分别强杀 client/JVM/机器；恢复不重放动作。（2026-09-22：**「不重放」那一半实现了、也有用例了**——`tests/fixtures/cases/host-050.json`，三条断言全部来自 `domain/recovery.py` 那张表与它的用例：只有可重复的 effect 可重试、「永不重放」的 effect 之后也不再被捡起、以及三条冻结期望与策略实际所做的一致；`run_repo_case.py` 跑出 `PASS` 3/3。**三种强杀本身仍要真实运行**（那也是这条用例的名字），所以 `mandatory` 仍是 `false`。）
7. `HOST-060`：磁盘接近配额、真正写满、备份目标满、日志目录满；不覆盖已有好备份。（2026-09-22：**路径那一半实现了**，`adapters/launcher/hosted_backup.py`，用例 `HOST-060` 三条断言、`run_repo_case.py` 跑出 `PASS`。**为什么不覆盖一份「验证过」的备份是安全规则而不是洁癖**：备份目标名基本是固定的（排程就是这么产生的），所以「再跑一次同一个世界的备份」是一件很自然的事——而如果那一份 save 在这期间坏掉了，它就**用一份坏副本盖掉最后一份好副本**，而那正是备份存在的理由。**未验证的残留可以被替换**（中断的备份留下的就是它，要求人工处理每一次中断是另一种坏）；**名字规则与 level name 共用一处**（含 `.publishing-` 前缀本身：以暂存前缀命名的备份会被清理暂存的那套东西扫走，而且两个备份会指向同一份字节）；**发布方式是「同目录内改名」**（与 `saves.py` 的 seeding 同一套做法），因为一次 rename 是唯一不会让读者看到半个备份的发布，摘要取的是**发布出去的那份字节**。**仍未做的**：磁盘接近配额/写满/备份目标满/日志目录满这四态（要真实磁盘与运行），以及轮换与配额——契约点名了它们而**没有给策略**，所以这里一条都没编。)
8. `HOST-070`：冷备→新目录恢复→实际进入并核对抽样状态；原世界不被修改。（2026-09-22：**路径那一半实现了**，同一个模块，用例 `HOST-070` 三条断言、`run_repo_case.py` `PASS`。**这一条防的是「从备份恢复」变成「把备份写回自己」**：目标在**被恢复的那棵树里面**（或就是它）一律拒绝，所以原件在恢复失败之后仍然在（契约第 6 条要的正是这个）；目标还必须是**某个 hosted world 自己的 `save/`**——它复用 `HOST-020` 那道 `admit_save_root` 的准入与它的原因码，因为「另一个 Kin 的区域」「越出 data root」「中间有 symlink」是**不同的错误**，这个模块不该把它们再拼一遍。**「写入哪个世界」是参数而不是默认**：回滚把一份备份恢复到**同一个世界**（新 epoch，见 `world_identity`），分叉恢复到**一个原本不存在的世界**，所以调用方先解出目标世界、函数再问这条路径是不是它的 `save/`。另一处是**原因码的先后**：目标既是来源又已存在时，报的是「目标就是来源」——两条都成立，但只有一条告诉操作者要改什么。**仍未做的**：真正恢复一次并进去抽样核对（要真实客户端），因此 `mandatory` 仍是 `false`；世界身份加 epoch 那一半早就在 `domain/world_identity.py` 里（`HOSTCOMMIT-110` 钉着）。)
9. `HOST-080`：hosted→remote→hosted；身份连续，两个 world context无背包/人物/计划串线。（2026-09-22：**「不串线」那一半实现了、也有用例了**——`tests/fixtures/cases/host-080.json`，四条断言：切换世界时**任何一步都只有一个 current**、另一个世界不能在已有 current 时成为 current、没走完的切换**什么都不留下**（而不是让代码去发明一个 current）、以及一个「两个都标 ACTIVE」的记录**被拒绝而不是被挑一个**；`run_repo_case.py` `PASS` 4/4。**「身份连续」那一半**（同一 Kin 跨 world context 不重建）与**整条 hosted→remote→hosted 路线**仍要真实运行，故 `mandatory` 仍为 `false`。）

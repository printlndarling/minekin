...
可以是写错的。**判据是变异，不是文本搜索**：把某个 `return` 的理由 neuter（`return None`，
或在不能落空处折成 0）之后跑全量，红了才说明有测试盯着它。文本搜索只用来**提名**候选——
它两个方向都错过一次：测试把 f-string 折行会让**已覆盖**的分支看起来没覆盖，而同一个理由名
被**两个函数**返回时，给其中一个写了测试会让另一个**看起来**已覆盖。2026-09-23 的读数是
**120 个不同的理由名、154 处 `return`，其中 0 个理由名**是测试文件里从未出现过的。
**但这个「0」不能读成「每条分支都有测试」**：文本审计只答「这个理由名出现过没有」，要答
「每条分支都有测试盯着」，只有对 154 处 `return` **逐个变异**（neuter 后跑测试，红了才算）。
**所以下一件工作是逐分支变异扫描**，重点是那 15 个被多个函数共用的理由名。口径、方法与
那两次方法错误都记在 `development-todo.md`。这一类不需要 Minecraft、不需要 runner、
不需要任何决定，可以现在做。

...
2026-09-23 的读数是 **154 处 `return`，144 处执行过、10 处从未执行**，而**这 10 处全部经
变异确认没有测试观察**——其中 **7 处**正是文本搜索判成「已覆盖」的，**6 处是
`LEDGER_UNREADABLE`**（同一个理由名被 11 个函数返回）。口径、方法与文本搜索的两次方法
错误都记在 `development-todo.md`；追踪的盲点是子进程追不到，所以「从未执行」只算提名。
**审计已入库为 `tools/trace_reason_branches.py`**（`uv run --frozen python tools/trace_reason_branches.py`，
约 3 分钟），不必重写。这一类不需要 Minecraft、不需要 runner、不需要任何决定，可以现在做。

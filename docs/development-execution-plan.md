...
| `E-RR` 修好的 runner 真跑复跑（`CORE-030` 加入尝试） | **`NEXT`（E lane 的 `lane_next` 附加卡，2026-09-26 已派工到 `../minekin-wt-evidence`，base 为 `main = be4e79b`：仅在分支、尚未验证）** | 把 ③ 与 G1/G2 从「代码 + 契约测试」升到真跑层证据：先复现第四刀那次静默 `rc=2` 的形状（全新加入者 Kin、`run/session/` 不存在），确认现行 runner 给的是具名读数而不是 0 字节 stderr；若修后的自动加入路径确实能走到封存，就按第四刀的形状做一次当前构建 LAN 真跑并封进规范卷（`sequence +1` 的新 attempt，不覆盖既成四条 seq 1/2、不改判据、不翻 registry），交四读 + 一条正对照 + 一条反证；拿不到 PASS 就停在具名前沿、按 `BLOCKED_HARNESS` 上报并保留全部失败材料。规范卷仍 E 独占写、M 只读；本卡不声称任何门晋级。 |
| `V1201-CONTROLLED-SERVER-STATUS-KNOB-001`（H2） | **`NEXT`（H lane 的 `lane_next`，2026-09-26 已派工到新支 `codex/minekin-controlled-server-status` @ `../minekin-wt-status`，base `be4e79b`：仅在分支、尚未验证）** | G1/G2 记录点名的遗留①：`tools/run_controlled_server.py:247` 仍无条件写 `enable-status=false`，也没有「实际写了什么」的回读——受控启动器因此**按构造**对 status 探测失明（G2 只能在 `domain.sh` 侧加读数与早停来绕过）。本卡把取舍变成显式、可命名的旋钮：**默认仍是 `false`**（不让任何已封 run 的语义搬家），加了 opt-in 之后要打印并回读它真正写进 `server.properties` 的值，且在任何写入之前按既有 loopback/地址规则具名拒绝不安全请求。测试必须钉住默认仍是 `false`（这条就是非空转保证：谁翻默认谁红）、opt-in 写 `true`、回读路径、具名拒绝，四者各自对改动前的副本量红。禁止挂载规范卷（此刻归 E 的 `E-RR`）；不改 `domain.sh`、不改判据/registry/门。 |

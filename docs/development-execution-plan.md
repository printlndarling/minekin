...
可以是写错的。用 `ast` 把该文件**所有函数**的 `return` 字面量与 f-string 前缀取出来，逐个去
`tests/unit/test_case_evidence_assertions.py` 里按「出现过」比对——测试不直接调断言函数，而是读
`verdict.failures` 里 `"<名字>:<原因>"` 这个形状，所以「出现过」就是「有测试断言过这条拒绝」。
2026-09-23 的读数是 **120 条不同的理由、154 处 `return`，其中 10 条理由没有任何测试让它的
分支响过一次**。这一类同样不需要 Minecraft、不需要 runner、不需要任何决定，可以现在做；
**那 10 条与可重推的审计口径都记在 `development-todo.md` 最近一条里**。

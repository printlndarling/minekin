...
| `PARALLEL-INTEGRATION-GATE-001` | **主干 `NEXT`（M，2026-09-26 随 lane_next 激活常驻）** | 主控 integration gate：按协议 §5 与 M1，逐卡双审 S/E/H/D/V 分支（允许路径、契约、测试、当前 build 真实证据），按依赖顺序每次只合一张卡入 `main`，合后核远端 SHA 并向各 lane 发新 base；分支未合并不得写成主干 `DONE`。本卡随激活持续有效，不是 docs-only 轮次卡。已合：S1 → 主干 `b88e36f`；V1 → 主干 `eb6f63d`（均 2026-09-26，双审+全量基础门后）。 |

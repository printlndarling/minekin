...
public final class ClientAdmissionController {
    ...
    /**
     * Vanilla's own abort wording, in the exact form vanilla's Cancel button
     * sends. {@code ConnectScreen.ABORTED_TEXT} is public in 1.21.4 and
     * package-private in 1.20.1, and both build it as
     * {@code Text.translatable("connect.aborted")} — measured from each
     * version's class file, which is what makes rebuilding it here equivalent.
     */
    private static final Text ABORTED_TEXT = Text.translatable("connect.aborted");
    ...
    static boolean clientIsLoaded(MinecraftClient client) {
        // Measured the hard way on 1.21.4 — testing `currentScreen != null` here was
        // already true during the load, so the hold never fired and the failure looked
        // unchanged. 1.20.1 has no `finishedLoading` flag to read instead (measured: no
        // such field or method on its MinecraftClient), and it represents the same
        // window as the loading overlay installed by `setOverlay`: the startup
        // SplashOverlay, and a progress overlay during a resource reload.
        //
        // That is a different predicate, not a renaming: it is also false while a pack
        // reload is in front of an already-loaded client. Untreated here as an
        // assumption — whether a 1.20.1 client held this way joins cleanly is a V04
        // measurement, not a compile-time fact.
    ...
    private void startWhenReady(MinecraftClient client) {
        ...
        ServerInfo server = new ServerInfo(
                // 1.20.1's ServerInfo has no server type at all; its third argument
                // is `local`, meaning LAN-discovered. This is a manually addressed
                // remote server, which is what OTHER says in 1.21.4.
                command.getServerProfileId(), address.toString(), false);
        ...
        LOGGER.info(
                "bridge asked vanilla to connect to {} for generation {} (loaded={}, "
                        + "screen={}, overlay={})",
                address,
                command.getGeneration(),
                clientIsLoaded(client),
        ...
        try {
            ...
            ConnectScreen.connect(
                    ...
                    address,
                    server,
                    // 1.20.1's connect path has no cookie-storage argument, so the
                    // handshake intent is not a choice this call makes: it is
                    // always LOGIN, which is what 1.21.4 asks for by passing null.
                    false);
    ...
    private void cancelVanilla(MinecraftClient client) {
        ...
        if (client.getNetworkHandler() != null) {
            client.getNetworkHandler().getConnection().disconnect(ABORTED_TEXT);
        }

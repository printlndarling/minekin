...
优先支持单客户端、单 AI 玩家、Minecraft Java **原版生存玩法**与私人测试服务器；Fabric 用于接入 Kin 客户端，不代表首版支持模组内容。模组玩法和多 Kin 作为 future。先证明自治与社会记忆，再扩大技能库和世界复杂度。所有动作最终经过正常客户端的移动、视角、使用、攻击等路径，不使用传送、无挖掘延迟或读取未观察世界的捷径。公开服务器部署前应遵守对应服务器的规则，并清楚标识这是自动化玩家。

...
> 状态：P0/W00 工程骨架开发中。当前代码只冻结工具链、模块边界与协议/测试接口，尚未证明真实客户端启动、入服或自治能力；所有运行能力仍以 evidence 门禁为准。
...
flowchart TD
    ...
    M --> S["Launcher / Session Manager"]
    S --> P["Probe / Version Bundle"]
    S --> MC["受管理的隐藏 Minecraft Client"]
    MC --> B["薄 Fabric Client Bridge"]
    B --> M
...
感知以玩家等价为基准，允许声明少量限域、限时的数据读取例外来解决成本或本地操作难题；例外不能偷偷用于找矿或社会判断，底层寻路同样要检查。可配置的矿透则是另一种由 Kin 基于人格和经历**主动启用**的玩家行为，发现须标明来源与后果，首版默认关闭。默认使用受限的本地结构化观察，**外部 VLM 默认关闭**；截图主要用于离线公平校准，只在确有复杂语义任务且已配置预算/隐私权限时按需调用，不持续上传、也不进入紧急控制闭环。见[屏幕观察契约](docs/screen-observation-contract.md)。见[感知政策](docs/perception-policy.md)。
...
- [数据根与身份创建：提案（未批准）](docs/run-directory-proposal.md)：`MINEKIN_HOME`/`MINEKIN_USERNAME` 与目录布局。契约都没有规定数据根位置，本文是提案而非契约。
